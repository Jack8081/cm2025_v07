...
)
{
	...
	case DISK_ENTER_HIGH_SPEED:

		if (disk_access_ioctl(disk_volume_strs[pdrv], DISK_IOCTL_ENTER_HIGH_SPEED, buff) != 0) {
			result = RES_ERROR;
		}
		break;

	case DISK_EXIT_HIGH_SPEED:
		if (disk_access_ioctl(disk_volume_strs[pdrv], DISK_IOCTL_EXIT_HIGH_SPEED, buff) != 0) {
			result = RES_ERROR;
		}
		break;
