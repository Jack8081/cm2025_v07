...
static void lv_roller_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
    ...
    if(code == LV_EVENT_GET_SELF_SIZE) {
        ...
        if (roller->option_cnt > 0) {
            const lv_font_t* font = lv_obj_get_style_text_font(obj, LV_PART_MAIN);
            lv_coord_t line_space = lv_obj_get_style_text_line_space(obj, LV_PART_MAIN);
            lv_obj_set_height(label, lv_font_get_line_height(font) * roller->option_cnt +
                line_space * (roller->option_cnt - 1));
        }
...
static void lv_roller_label_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
    ...
    if(code != LV_EVENT_DRAW_MAIN && code != LV_EVENT_STYLE_CHANGED && code != LV_EVENT_GET_SELF_SIZE) {
        /* Call the ancestor's event handler */
    ...
    /*Post draw when the children are drawn*/
        ...
        if(area_ok) {
            ...
            if (roller_data->option_chr) {
                ...
                lv_coord_t y_off = mask_sel.y1 - label_sel_area.y1;
                ...
                label_sel_area.y1 += opt_y_off;
                if(opt_idx >= 0 && opt_idx < roller_data->option_cnt)
                    lv_draw_label(&label_sel_area, &mask_sel, &label_dsc, roller_data->option_chr[opt_idx], NULL);
            } else {
...
static void draw_label(lv_event_t * e)
{
    ...
    if(_lv_area_intersect(&clip2, clip_area, &clip2)) {
        ...
        if (roller_data->option_chr) {
            lv_coord_t font_h = lv_font_get_line_height(label_draw_dsc.font);
            lv_coord_t line_h = font_h + label_draw_dsc.line_space;
            lv_coord_t y_off = clip2.y1 - label_obj->coords.y1;
            lv_coord_t opt_idx = y_off / line_h;
            lv_coord_t opt_y_off = opt_idx * line_h;
            if (y_off - opt_y_off >= font_h) {
                opt_y_off += line_h;
            ...
            label_obj->coords.y1 += opt_y_off;
            if(opt_idx >= 0 && opt_idx < roller_data->option_cnt)
                lv_draw_label(&label_obj->coords, &clip2, &label_draw_dsc, roller_data->option_chr[opt_idx], NULL);
            label_obj->coords.y1 -= opt_y_off;
        } else {
