...
/*LV_IMG_CF_ARGB_6666 to ARGB_8888*/
static inline uint32_t lv_color6666_to32(const uint8_t color[3])
{
    lv_color32_t c32;

    c32.ch.blue  = (color[0] << 2) | (color[0] & 0x3);
    c32.ch.green = (color[1] << 4) | ((color[0] >> 4) & 0x0c) | (color[0] >> 6);
    c32.ch.red   = (color[2] << 6) | ((color[1] >> 2) & 0x3c) | ((color[1] >> 4) & 0x03);
    c32.ch.alpha = (color[2] & 0xfc) | ((color[2] & 0x0c) >> 2);
