...
LV_ATTRIBUTE_FAST_MEM static void lv_draw_map(const lv_area_t * map_area, const lv_area_t * clip_area,
                                              const uint8_t * map_p,
                                              const lv_draw_img_dsc_t * draw_dsc,
                                              bool chroma_key, lv_img_cf_t cf);
                                                       ...
                                                       const lv_draw_img_dsc_t * draw_dsc)
{
     ...
     *Just draw it!*/
        ...
        lv_draw_map(coords, &mask_com, cdsc->dec_dsc.img_data, draw_dsc, chroma_keyed, /*alpha_byte*/cdsc->dec_dsc.header.cf);
    }
    /*The whole uncompressed image is not available. Try to read it line-by-line*/
        ...
        for(row = mask_com.y1; row <= mask_com.y2; row++) {
            ...
            lv_draw_map(&line, &mask_line, buf, draw_dsc, chroma_keyed, /*alpha_byte*/cdsc->dec_dsc.header.cf);
            line.y1++;
            line.y2++;
            y++;
        }
...
LV_ATTRIBUTE_FAST_MEM static void lv_draw_map(const lv_area_t * map_area, const lv_area_t * clip_area,
                                              const uint8_t * map_p,
                                              const lv_draw_img_dsc_t * draw_dsc,
                                              bool chroma_key, lv_img_cf_t cf/*bool alpha_byte*/)
{
    bool alpha_byte   = lv_img_cf_has_alpha(cf);
    /* FIXME: the decoding buffer will only be LV_IMG_CF_TRUE_COLOR or LV_IMG_CF_TRUE_COLOR_ALPHA ? */
    if (cf != LV_IMG_CF_ARGB_8888 && cf != LV_IMG_CF_ARGB_6666) {
        cf = alpha_byte ? LV_IMG_CF_TRUE_COLOR_ALPHA : LV_IMG_CF_TRUE_COLOR;
    }
    ...
    /*In the other cases every pixel need to be checked one-by-one*/
        ...
        uint8_t px_size_byte = lv_img_cf_get_px_size(cf) >> 3;
        ...
        if(!mask_any && !transform && !chroma_key && draw_dsc->recolor_opa == LV_OPA_TRANSP && alpha_byte) {
#if LV_USE_GPU
            lv_img_cf_t tmp_cf = cf;
#if LV_COLOR_DEPTH == 32
            if(cf != LV_IMG_CF_ARGB_6666)
                tmp_cf = LV_IMG_CF_ARGB_8888;
#endif
            if(tmp_cf == LV_IMG_CF_ARGB_8888 || tmp_cf == LV_IMG_CF_ARGB_6666) {
                /*Blend ARGB images directly*/
                if(disp->driver->gpu_blend_cb && (lv_area_get_size(&draw_area) > LV_GPU_SIZE_LIMIT)) {
                    int32_t disp_w = lv_area_get_width(disp_area);
                    lv_color_t * disp_buf = draw_buf->buf_act;
                    lv_color_t * disp_buf_first = disp_buf + disp_w * draw_area.y1 + draw_area.x1;
                    if(disp->driver->gpu_blend_cb(disp->driver, disp_buf_first, disp_w, (const lv_color_t *)map_buf_tmp,
                                              map_w, tmp_cf, draw_area_w, draw_area_h, draw_dsc->opa) >= 0) {
                        return;
                    }
                }
            }
            ...
            if(disp->driver->sw_blend_cb && draw_dsc->opa > LV_OPA_MAX) {
                int32_t disp_w = lv_area_get_width(disp_area);
                lv_color_t * disp_buf = draw_buf->buf_act;
                lv_color_t * disp_buf_first = disp_buf + disp_w * draw_area.y1 + draw_area.x1;
                disp->driver->sw_blend_cb(disp->driver, disp_buf_first, disp_w, map_buf_tmp,
                                          map_w, cf, draw_area_w, draw_area_h);
                return;
            }
            ...
            for(y = 0; y < draw_area_h; y++) {
                ...
                for(x = 0; x < draw_area_w; x++, map_px += px_size_byte, px_i++) {
                    lv_opa_t px_opa = map_px[/*LV_IMG_PX_SIZE_ALPHA_BYTE*/px_size_byte - 1];

                    lv_color32_t c32;
                    if(cf == LV_IMG_CF_ARGB_6666) {
                        c32.full = lv_color6666_to32(map_px);
                        px_opa = c32.ch.alpha;
                    }
                    ...
                    if(px_opa) {
                        if(cf == LV_IMG_CF_ARGB_6666) {
                            map2[px_i] = lv_color_make(c32.ch.red, c32.ch.green, c32.ch.blue);
                        } else {
#if LV_COLOR_DEPTH == 8 || LV_COLOR_DEPTH == 1
                            if(cf == LV_IMG_CF_ARGB_8888) {
                                map2[px_i] = lv_color_make(map_px[2], map_px[1], map_px[0]);
                            } else {
            ...
            for(y = 0; y < draw_area_h; y++) {
                ...
                for(x = 0; x < draw_area_w; x++, map_px += px_size_byte, px_i++) {
...
#endif
                    {
                        ...
                        if(cf == LV_IMG_CF_ARGB_6666) {
                            c = lv_color_make(c32.ch.red, c32.ch.green, c32.ch.blue);
                        } else {
#if LV_COLOR_DEPTH == 1
                            if (cf == LV_IMG_CF_ARGB_8888) {
                                c = lv_color_make(map_px[2], map_px[1], map_px[0]);
                            } else {
                                c.full = map_px[0];
                            }
