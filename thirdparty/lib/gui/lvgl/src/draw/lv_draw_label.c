                                          ...
                                          lv_color_t color, lv_opa_t opa, lv_blend_mode_t blend_mode)
{
    ...
    if (disp->driver->draw_letter_cb && blend_mode == LV_BLEND_MODE_NORMAL) {
        disp->driver->draw_letter_cb(disp->driver, pos_x, pos_y, &g, clip_area, map_p, color, opa);
    } else if(font_p->subpx) {
#if LV_DRAW_COMPLEX && LV_USE_FONT_SUBPX
