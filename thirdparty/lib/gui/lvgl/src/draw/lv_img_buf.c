...
lv_color_t lv_img_buf_get_px_color(lv_img_dsc_t * dsc, lv_coord_t x, lv_coord_t y, lv_color_t color)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_ARGB_8888) {
...
void lv_img_buf_set_px_alpha(lv_img_dsc_t * dsc, lv_coord_t x, lv_coord_t y, lv_opa_t opa)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA || dsc->header.cf == LV_IMG_CF_ARGB_8888) {
        uint8_t px_size          = lv_img_cf_get_px_size(dsc->header.cf) >> 3;
        uint32_t px              = dsc->header.w * y * px_size + x * px_size;
...
void lv_img_buf_set_px_color(lv_img_dsc_t * dsc, lv_coord_t x, lv_coord_t y, lv_color_t c)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_ARGB_8888) {
        ...
        lv_memcpy_small(&buf_u8[px], &c32, 3); /*-1 to not overwrite he alpha value*/
    }
    else if(dsc->header.cf == LV_IMG_CF_ARGB_6666) {
        uint32_t px      = dsc->header.w * y * 3 + x * 3;
        lv_color32_t c32 = { .full = lv_color_to32(c), };
