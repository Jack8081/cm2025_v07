                                              ...
                                              const lv_opa_t * mask, lv_draw_mask_res_t mask_res)
{
    ...
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
#if LV_USE_GPU
        if(disp->driver->gpu_fill_cb && (lv_area_get_size(draw_area) > LV_GPU_SIZE_LIMIT)) {
            if(disp->driver->gpu_fill_cb(disp->driver, disp_buf_first, disp_w, color, opa, draw_area_w, draw_area_h) >= 0) {
                return;
            }
        }
...
#if LV_USE_GPU
#if 0
        if(disp->driver->gpu_fill_mask_cb && (lv_area_get_size(draw_area) > LV_GPU_SIZE_LIMIT)) {
            if(disp->driver->gpu_fill_mask_cb(disp->driver, disp_buf_first, disp_w, color, opa, mask,
                    draw_area_w, LV_IMG_CF_ALPHA_8BIT, draw_area_w, draw_area_h) >= 0) {
                return;
            }
        }
#endif
                                             ...
                                             const lv_opa_t * mask, lv_draw_mask_res_t mask_res)
{
    ...
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
        if(opa > LV_OPA_MAX) {
 #if LV_USE_GPU
            if(disp->driver->gpu_copy_cb && (lv_area_get_size(draw_area) > LV_GPU_SIZE_LIMIT)) {
                if(disp->driver->gpu_copy_cb(disp->driver, disp_buf_first, disp_w, map_buf_first,
                                         map_w, draw_area_w, draw_area_h) >= 0) {
                    return;
                }
            }
            ...
            if(disp->driver->gpu_blend_cb && (lv_area_get_size(draw_area) > LV_GPU_SIZE_LIMIT)) {
                if(disp->driver->gpu_blend_cb(disp->driver, disp_buf_first, disp_w, map_buf_first,
                                          map_w, LV_IMG_CF_TRUE_COLOR, draw_area_w, draw_area_h, opa) >= 0) {
                    return;
                }
            }
