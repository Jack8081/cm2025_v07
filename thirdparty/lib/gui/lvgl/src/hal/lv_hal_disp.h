...
typedef struct _lv_disp_drv_t {
    ...
    /**< 1: previous drawing to the internal buffer (VDB) using gpu*/
    uint32_t using_gpu : 1;
...
#else
    /** OPTIONAL: called to trace the beginning of refreshing */
    void (*refresh_start_cb)(struct _lv_disp_drv_t * disp_drv);

    /** OPTIONAL: called to trace the beginning of drawing the internal buffer (VDB) */
    void (*prepare_cb)(struct _lv_disp_drv_t * disp_drv, const lv_area_t * area);

    /** OPTIONAL: Custom software implemention of blending two memories without opacity*/
    void (*sw_blend_cb)(struct _lv_disp_drv_t * disp_drv, lv_color_t * dest, lv_coord_t dest_w, const void * src,
                        lv_coord_t src_w, lv_img_cf_t src_cf, lv_coord_t copy_w, lv_coord_t copy_h);

    /** OPTIONAL: Custom software implemention of drawing letter*/
    void (*draw_letter_cb)(struct _lv_disp_drv_t * disp_drv, lv_coord_t pos_x, lv_coord_t pos_y, lv_font_glyph_dsc_t * g,
                        const lv_area_t * clip_area, const uint8_t * map_p, lv_color_t color, lv_opa_t opa);

    /** OPTIONAL: Fill a memory with a color (GPU only)
     * return non-negative value on success else negative errno code.
     **/
    int (*gpu_fill_cb)(struct _lv_disp_drv_t * disp_drv, lv_color_t * dest, lv_coord_t dest_w,
                        lv_color_t color, lv_opa_t opa, lv_coord_t fill_w, lv_coord_t fill_h);

    /** OPTIONAL: Fill a memory with a color using the overall opacity and the optional pixel mask
     *    which describes the opacity of each pixel (GPU only)
     * return return non-negative value on success else negative errno code.
     **/
    int (*gpu_fill_mask_cb)(struct _lv_disp_drv_t * disp_drv, lv_color_t * dest, lv_coord_t dest_w,
                        lv_color_t color, lv_opa_t opa, const void * mask, lv_coord_t mask_w,
                        lv_img_cf_t mask_cf, lv_coord_t fill_w, lv_coord_t fill_h);

    /** OPTIONAL: Copy memory to another memory using opacity and the optional pixel color format conversion (GPU only)
     * return return non-negative value on success else negative errno code.
     **/
    int (*gpu_copy_cb)(struct _lv_disp_drv_t * disp_drv, lv_color_t * dest, lv_coord_t dest_w, const void * src,
                        lv_coord_t src_w, lv_coord_t copy_w, lv_coord_t copy_h);

    /** OPTIONAL: Blend two memories using opacity and the optional pixel color format conversion  (GPU only)
     * return return non-negative value on success else negative errno code.
     **/
    int (*gpu_blend_cb)(struct _lv_disp_drv_t * disp_drv, lv_color_t * dest, lv_coord_t dest_w, const void * src,
                        lv_coord_t src_w, lv_img_cf_t src_cf, lv_coord_t copy_w, lv_coord_t copy_h, lv_opa_t opa);

    /** OPTIONAL: Configure rotating a "ring" memory to another memory with color filling to the circular hole of the "ring" (GPU only)
     * return return non-negative value on success else negative errno code.
     **/
    int (*gpu_rotate_config_cb)(struct _lv_disp_drv_t * disp_drv, lv_coord_t outer_diameter,
                       lv_coord_t inner_diameter, uint16_t angle, lv_color_t fill_color);

    /** OPTIONAL: Start rotating a part of "ring" memory to another memory with color filling to the circular hole of the "ring" (GPU only)
     * return return non-negative value on success else negative errno code.
     **/
    int (*gpu_rotate_start_cb)(struct _lv_disp_drv_t * disp_drv, lv_color_t * dest, lv_coord_t dest_w,
                        const lv_color_t * src, lv_coord_t row_start, lv_coord_t row_count);
#endif
...
#if LV_DISP_TOP_LAYER
    struct _lv_obj_t * top_layer;   /**< @see lv_disp_get_layer_top*/
#endif
#if LV_DISP_SYS_LAYER
