...
/* Join the y-intersectant inv areas to avoid screen tearing */
#define JOIN_Y_INTERSECTANT_INV_AREAS 1
...
#if JOIN_Y_INTERSECTANT_INV_AREAS
...
#endif /* JOIN_Y_INTERSECTANT_INV_AREAS */
...
void _lv_disp_refr_timer(lv_timer_t * tmr)
{
    ...
    if(disp_refr->inv_p != 0 && disp_refr->driver->refresh_start_cb) {
        disp_refr->driver->refresh_start_cb(disp_refr->driver);
    }
...
static void lv_refr_join_area(void)
{
#if JOIN_Y_INTERSECTANT_INV_AREAS == 0
    uint32_t join_from;
    uint32_t join_in;
...
static void lv_refr_area_part(const lv_area_t * area_p)
{
    ...
    if(disp_refr->driver->prepare_cb) disp_refr->driver->prepare_cb(disp_refr->driver, &start_mask);
...
static void lv_refr_obj(lv_obj_t * obj, const lv_area_t * mask_ori_p)
{
    ...
    if(union_ok != false) {
#if LV_STRACE_OBJ
        LV_STRACE_U32X5(LV_STRACE_ID_OBJ_DRAW, (uintptr_t)obj,
            obj_ext_mask.x1, obj_ext_mask.y1, obj_ext_mask.x2, obj_ext_mask.y2);
#endif
...
#if LV_STRACE_OBJ
        LV_STRACE_CALL_U32(LV_STRACE_ID_OBJ_DRAW, (uintptr_t)obj);
#endif
    }
}
...
#if LV_USE_GPU
    /* Let flush_cb() decide whether gpu_wait_cb() is required
