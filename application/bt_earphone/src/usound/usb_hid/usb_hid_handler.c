...
LOG_MODULE_REGISTER(usb_hid_handler);
				...
				u8_t **data)
{
	...
	LOG_DBG("temp_data[0]: 0x%04x", temp_data[0]);
	LOG_DBG("temp_data[1]: 0x%04x", temp_data[1]);
	LOG_DBG("temp_data[2]: 0x%04x", temp_data[2]);

	recv_report = (temp_data[2] << 16) | (temp_data[1] << 8) | temp_data[0];

	LOG_DBG("recv_report:0x%04x", recv_report);
...
static void hid_inter_out_ready(void)
{
	...
	ret = hid_int_ep_read(usb_hid_out_buf, sizeof(usb_hid_out_buf),
			&bytes_to_read);
	if (!ret) {
		LOG_DBG("bytes_to_read:%d", bytes_to_read);
		switch (usb_hid_out_buf[0]) {
		case HID_SELF_DEFINED_KEY_ID:
			LOG_DBG("self-defind-key:");
			for (u8_t i = 0; i < bytes_to_read; i++) {
				LOG_DBG("usb_hid_out_buf[%d] = 0x%02x", i, usb_hid_out_buf[i]);
			}
			break;

		case HID_SELF_DEFINED_DAT_ID:
			LOG_DBG("sel-defined-data:");
			for (u8_t i = 0; i < bytes_to_read; i++) {
				LOG_DBG("usb_hid_out_buf[%d] = 0x%02x", i, usb_hid_out_buf[i]);
			}
			break;
		...
		LOG_ERR("read output report error");
		return;
	}
}
...
static int usb_hid_tx(const u8_t *buf, u16_t len)
{
	...
	if (ret) {
		...
		LOG_ERR("wrote: %d, len: %d", wrote, len);
	}
...
int usb_hid_tx_self_defined_key(const u8_t *buf, u8_t len)
{
	...
	if (ret == 0) {
		...
		LOG_ERR("hid tx self-defined key error");
	}
	return ret;
}
