...
{
    test_result_e ret_val = 0;
    output_arg_t bt_name_arg[2];
    char bt_dev_name[CFG_MAX_BT_DEV_NAME_LEN +1];
    char ble_dev_name[CFG_MAX_BT_DEV_NAME_LEN +1];

    act_test_read_arg(arg_buffer, &bt_name_arg, 2);

    if (bt_name_arg[0].arg_length < sizeof(bt_dev_name) && bt_name_arg[1].arg_length < sizeof(ble_dev_name))
    {
        memset(bt_dev_name, 0, sizeof(bt_dev_name));
        memset(ble_dev_name, 0, sizeof(ble_dev_name));
        memcpy(bt_dev_name, bt_name_arg[0].arg, bt_name_arg[0].arg_length);
        memcpy(ble_dev_name, bt_name_arg[1].arg, bt_name_arg[1].arg_length);
        //att_buf_printf("bt_dev_name:%s, ble_dev_name: %s\n", bt_dev_name, ble_dev_name);

        ret_val += act_test_set_bt_name(bt_dev_name, strlen(bt_dev_name));
        ret_val += act_test_set_le_audioname(ble_dev_name, strlen(ble_dev_name));
