...
static bool att_cfo_read_test(_bt_mp_test_arg_t *mp_arg, u8_t cap_value)
{
    ...
    for(i = 0; i < 3; i++) {
        ...
        if (cfo_val[i] < mp_arg->cfo_low_limit || cfo_val[i] > mp_arg->cfo_high_limit) {
            LOG_E("mp read test fail,channel:%d,cfo:%d\n", mp_arg->channel[i], cfo_val[i]);
            ret = false;
            goto test_end;
        }
    }
...
bool att_check_need_adjust_cfo(void)
{
	...
	if(ret_val == TRIM_CAP_READ_EFUSE_FULL) {
		att_buf_printf("efuse position full, return!\n");
		ret_val = true;
	} else if (ret_val == TRIM_CAP_READ_NO_ERROR) {
...
bool __ENTRY_CODE pattern_main(struct att_env_var *para)
{
	...
	//set register
	sys_write32((sys_read32(HOSC_CTL) & (0xffff0000)) | 0x7730,  HOSC_CTL);
	sys_write32((sys_read32(HOSCLDO_CTL) & (0xffff0000)) | 0x8014,  HOSCLDO_CTL);

	//vdd
	//sys_write32(0x04ffee0b,  0x40004004);
...
test_end_0:
    ...
    att_buf_printf("att test end : %s\n", (ret_val==TEST_PASS)?"Pass":"Fail");
