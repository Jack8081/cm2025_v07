...
int rf_rx_cfo_test_channel(_bt_mp_test_arg_t *mp_arg, u32_t channel, s16_t ref_cap, s16_t *return_cap, s16_t *return_cfo)
{
    ...
    att_buf_printf("middle:%d, low:%d, high:%d\n", cfo_threshold_middle, cfo_threshold_low, cfo_threshold_high);
	...
	att_buf_printf("ref:%d, left:%d, right:%d\n", cap_index, left, right);

retry:

    try_count ++;

    att_buf_printf("cfo test cap_index : %d, cnt: %d, retry_cnt: %d\n", cap_index, try_count, test_retry_count);
...
#if 0
    if (ret_val < 0) {
        att_buf_printf("cfo test fail,try count:%d, ret:%d\n", try_count, ret_val);
        goto test_fail;
    }
#else
    ...
    att_buf_printf("cap_index is %d, cfo_val is %d\n", cap_index, cfo_val);
    ...
    if (cfo_val > cfo_threshold_middle) {
        if (cfo_val < last_positive_value) {
            ...
            if ((cfo_val - last_positive_value <= 1) && (cap_index - left < 2)) {
				...
				#if 0
                att_buf_printf("mp test fail, direction err,current cfo:%d,last positive cfo:%d, cap%d\n",
        ...
        if (cfo_val > last_negative_value) {
            ...
            if ((last_negative_value - cfo_val <= 1) && (right - cap_index < 2)) {
				...
				#if 0
                att_buf_printf("mp test fail, direction err,current cfo:%d,last negative cfo:%d, cap%d\n",
