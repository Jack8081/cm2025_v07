...
u8_t att_read_trim_cap(u32_t mode)
{
    ...
    if ((ret_val == TRIM_CAP_READ_NO_ERROR) || (ret_val == TRIM_CAP_READ_HAVE_SPACE)) {
        return trim_cap;
    } else {
...
u8_t att_write_trim_cap(u8_t index, u32_t mode)
{
	...
	if ((ret_val == TRIM_CAP_READ_NO_ERROR) || (ret_val == TRIM_CAP_READ_HAVE_SPACE)) { /*FIXME : return must differentiate between efuse and nvram*/
		if (my_abs(index - value_bak) <= 2) {
			trim_cap = value_bak;
		}
		LOG_I("old val %d set val %d new val %d", value_bak, index, trim_cap);
	}
#endif
