...
extern u16_t g_energy_level;
extern u16_t audio_track_energy_value(short *buf, int len, int bit_depth);
#endif
...
#ifdef CONFIG_UP_LOAD_ENERGY_DETECT
    g_energy_level = audio_track_energy_value((short*)buf, ret, CONFIG_USB_AUDIO_RESOLUTION);
#endif
	if (ret != len) {
		SYS_LOG_DBG("want read %d bytes ,but only read %d bytes \n",len,ret);
	}
...
int usb_audio_stream_init(io_stream_t handle, void *param)
{
	...
	acts_ringbuf_init(info->cache_buff,
				media_mem_get_cache_pool(USB_UPLOAD_CACHE, AUDIO_STREAM_TR_USOUND),
				media_mem_get_cache_pool_size(USB_UPLOAD_CACHE, AUDIO_STREAM_TR_USOUND));
...
int usb_audio_upload_stream_set(io_stream_t stream)
{
	...
	sys_irq_lock(&flags);
	usb_audio_upload_stream = stream;
	sys_irq_unlock(&flags);
