...
static void _usb_audio_stream_state_notify(u8_t stream_event)
{
    ...
    if(stream_event == USOUND_UPLOAD_STREAM_STOP)
    {
	    ...
	    os_delayed_work_submit(&usb_audio->upload_stream_notify_work, 2000);
    }
...
{
#ifndef CONFIG_TR_USOUND_START_STOP_CIS_DETECT
...
static int _usb_audio_check_stream(void *pcm_data, u32_t len)
{
    ...
    if (energy(pcm_data, len) == 0) {
		...
		if (usb_audio->play_state && usb_audio->zero_frame_cnt > 5000) {
			usb_audio->play_state = 0;
...
#ifndef CONFIG_TR_USOUND_DOUBLE_SOUND_CARD
...
void set_sound_card_soft_volume(int sound_vol, u8_t *in_buf, int len, int byte_depth)
{
    ...
    int mute = audio_system_get_stream_mute(AUDIO_STREAM_TR_USOUND);
	...
	enum usb_dc_ep_cb_status_code cb_status)
{
	...
	if (usb_audio_upload_stream) {
        ...
        {
            if(skip_frame_cnt < SKIP_FRAME_MAC_OS_CNT)//过滤前面上传的SKIP_FRAME_CNT ms的数据 mac os
            {
                skip_frame_cnt++;
        ...
        {
            if(skip_frame_cnt < SKIP_FRAME_CNT)//过滤前面上传的SKIP_FRAME_CNT ms的数据
            {
                skip_frame_cnt++;
                memset(usb_audio->usb_audio_play_load, 0, MAX_UPLOAD_BUF_SIZE);
            }
...
int usb_audio_init(usb_audio_event_callback cb)
{
	...
	audio_cur_level = audio_system_get_current_volume(AUDIO_STREAM_TR_USOUND);
    ...
    ret = usb_device_register_string_descriptor(DEV_STRING50_DESC, CONFIG_USB_APP_AUDIO_DEVICE_STRING50, strlen(CONFIG_USB_APP_AUDIO_DEVICE_STRING50));
	if (ret) {
		goto exit;
	}
