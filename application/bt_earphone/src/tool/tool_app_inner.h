...
#include <../subsys/trace/cbuf.h>

#include "tool_app.h"

//#define USP_WRITE_TIMEOUT_MS (10)
#define USP_WRITE_TIMEOUT_MS (0)


#ifdef CONFIG_TOOL_ASET_USB_SUPPORT
typedef enum {
    ASET_USB_STATUS_INIT = 0,
	ASET_USB_STATUS_HANDSHAKE_OK,
	ASET_USB_STATUS_RUNNING,
	ASET_USB_STATUS_FINISH
} aset_usb_status_e;
#endif
...
#ifdef CONFIG_TOOL_ASET_USB_SUPPORT
    uint32_t stub_install_timer_ms;
    os_timer stub_det_timer;
    uint8_t usb_in_flag;
	bool wait_stub_drv_ok_flag;
	bool aset_protocol_connect_flag;
    aset_usb_status_e aset_usb_status;
	uint8_t *stub_read_buf;
	uint8_t *stub_write_buf;
...
enum CONFIG_PROTOCOL_COMMANDS
{
    ...
    CONFIG_PROTOCOL_CMD_PCM_DATA  = 0x7c,
    CONFIG_PROTOCOL_CMD_STUB_STOP= 0x7f,

};


extern act_tool_data_t g_tool_data;
extern const char sysrq_toolbuf[7];
extern const uint8_t tool_ack[6];
...
int usb_stub_ep_write_async(u8_t *data_buffer, u32_t data_len, u32_t timeout);
#endif
