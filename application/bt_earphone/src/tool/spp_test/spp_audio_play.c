...
void audio_data_play_test_callback(struct thread_timer *ttimer, void *expiry_fn_arg)
{
    ...
    // thread_timer_start(ttimer, time_ms, 0);
}
...
void audio_data_play_test_start(s16_t *pcm_data, u32_t size, u32_t sample_rate)
{
    ...
    else
    {
        ...
        // hrtimer_init(&audio_play_test->timer, audio_data_play_test_callback, NULL);
        // hrtimer_start(&audio_play_test->timer, 1, 0);
...
void audio_data_play_test_stop(void)
{
    ...
    if (audio_play_test)
    {
        ...
        {
            // hrtimer_stop(&audio_play_test->timer);
