...
#define APP_OTA_VERSION_MAX	    (0xFFFFFFFF)	


#define MAX_SPPBLE_STREAM		(1)
...
static io_stream_t sppble_create_tws_stream[MAX_SPPBLE_STREAM] __IN_BT_SECTION;
...
static int sppble_add_stream(io_stream_t handle)
{
	...
	for (i = 0; i < MAX_SPPBLE_STREAM; i++) {
		if (sppble_create_tws_stream[i] == NULL) {
			sppble_create_tws_stream[i] = handle;
			break;
		}
	}
...
static int sppble_remove_stream(io_stream_t handle)
{
	...
	os_mutex_lock(&g_sppble_mutex, OS_FOREVER);
	for (i = 0; i < MAX_SPPBLE_STREAM; i++) {
		if (sppble_create_tws_stream[i] == handle) {
			sppble_create_tws_stream[i] = NULL;
			break;
		}
	}
	os_mutex_unlock(&g_sppble_mutex);
...
static io_stream_t find_streame_by_spp_uuid(uint8_t *uuid)
{
	...
	for (i = 0; i < MAX_SPPBLE_STREAM; i++) {
		stream = sppble_create_tws_stream[i];
		if (stream) {
...
static void stream_receive_data_from_slave_cb(u8_t channel, void *attr, u8_t pn, u8_t *data, u16_t len)
{
	struct sppble_info_t *info;
	io_stream_t handle;

	if (attr)
		handle = find_streame_by_ble_attr(attr);
	else
		handle = find_streame_by_spp_channel(channel);

	SYS_LOG_INF("channel:%d, len:%d", channel, len);
	if (handle && handle->data) {
		info = (struct sppble_info_t *)handle->data;
		if (info->connect_type != NONE_CONNECT_TYPE) {
			info->pn_slave = pn;
			SYS_LOG_INF("info->pn_slave:%d", info->pn_slave);
			if ((!info->slave_data) || (len > 64))
			{
				SYS_LOG_ERR("len:%d.",len);
				return;
			}

			memcpy(info->slave_data, data, len);
		}
	}
}
		...
		if (info->connect_type == NONE_CONNECT_TYPE) {
			...
			if (info->connect_cb) {
				...
				if (BTSRV_TWS_MASTER == bt_manager_tws_get_dev_role())
				{
					//input_manager_lock();
					info->tws_ota = 1;
					SYS_LOG_INF("info->tws_ota:%d", info->tws_ota);
					if (!info->slave_data)
					{
						info->slave_data = mem_malloc(64);
						if(!info->slave_data)
							return;
						info->pn_slave = 0;
						info->pn_master = 0;
						master_cb.connect_cb = info->connect_cb;
						master_cb.tws_receive_data = stream_receive_data_from_slave_cb;
						tws_ota_master_register(channel, NULL, &master_cb);
					}

					memset(&st, 0, sizeof(tws_ota_info_t));
					st.type = OTA_TWS_TYPE_CMD;
					st.cmd = OTA_BREAKPOINT;
	...
	if (handle && handle->data) {
		...
		if (info->connect_type == SPP_CONNECT_TYPE) {
			...
			if (info->connect_cb) {
				...
				if (info->slave_data)
				{
					mem_free(info->slave_data);
					info->slave_data = NULL;
				}
				info->pn_slave = 0;
				info->pn_master = 0;
				info->tws_ota = 0;
...
static void stream_spp_receive_data_cb(uint8_t channel, uint8_t *data, uint32_t len)
{
	...
	if (handle && handle->data) {
		...
		if (info->connect_type == SPP_CONNECT_TYPE) {
			...
			sppble_open(handle, 0);

			sppble_rx_date(handle, data, len);

			memset(&st, 0, sizeof(tws_ota_info_t));
			st.type = OTA_TWS_TYPE_DATA_SLAVE;
			st.ctx_len = len;
			tws_ota_transmit_data(&st, data);
		}
	}
}
			      ...
			      uint8_t flags)
{
	...
	if (stream && stream->data) {
		...
		if(!info->bt_addr || strncmp((uint8_t*)info->bt_addr, (uint8_t*)&tmp_btaddr, sizeof(bt_addr_t))){
		...
		if (info->connect_type == BLE_CONNECT_TYPE) {
			...
			st.type = OTA_TWS_TYPE_DATA_SLAVE;
			st.ctx_len = len;
			tws_ota_transmit_data(&st, (uint8_t *)buf);
		}
	}
...
static void stream_ble_rx_set_notifyind(const struct bt_gatt_attr *attr, uint16_t value)
{
	...
	if (info->notify_ind_enable) {
		if (info->connect_type == NONE_CONNECT_TYPE) {
            ...
            extern void set_data_chan_phone_value(u8_t pram);
            set_data_chan_phone_value(value);
            if(value == 0xf5)
            {
			...
			if (info->connect_cb) {
				if (BTSRV_TWS_MASTER == bt_manager_tws_get_dev_role())
				{
					...
					if (!info->slave_data)
					{
						...
						master_cb.connect_cb = info->connect_cb;
						master_cb.tws_receive_data = stream_receive_data_from_slave_cb;
						tws_ota_master_register(0xFF, (void *)attr, &master_cb);
					}
	...
	if (!connected) {
		for (i = 0; i < MAX_SPPBLE_STREAM; i++) {
			...
			if (stream) {
				...
				if(!info->bt_addr || strncmp((uint8_t*)info->bt_addr->val, mac, 6)){
					continue;
				}
				...
				if (info->connect_type == BLE_CONNECT_TYPE) {
					SYS_LOG_INF("BLE disconnect xxxx ");
...
static int sppble_write(io_stream_t handle, uint8_t *buf, int num)
{
	...
	info->pn_master++;
	SYS_LOG_INF("pn_slave %d pn_master %d", info->pn_slave,info->pn_master);

	while (info->connect_type != NONE_CONNECT_TYPE) {
		if (0 == info->tws_ota)
			break;
		...
		if (info->pn_slave == info->pn_master) {
			if (memcmp(buf, info->slave_data, num) != 0)
			{
				print_hex("slave: ", info->slave_data, num);
				print_hex("master: ", buf, num);
			}
			break;
		} else {
			if (timeout >= 7000) {
				SYS_LOG_ERR("get tws timeout!");
				os_mutex_unlock(&info->write_mutex);
				return -EIO;
			}
...
const stream_ops_t sppble_stream_tws_ops = {
	.init = sppble_init,
	.open = sppble_open,
	.read = sppble_read,
	.seek = NULL,
...
io_stream_t sppble_stream_tws_create(void *param)
{
	return stream_create(&sppble_stream_tws_ops, param);
}
#endif
