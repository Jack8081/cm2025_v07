...
#include "media_bt_dsp_xfer.h"
#include "media_audio_stream.h"
...
enum media_srv_operation_e {
	...
	/** pre start to play*/
	MSG_MEDIA_SRV_PRE_PLAY,
	/** pre enable, to pre open the algorithm library*/
	MSG_MEDIA_SRV_PRE_ENABLE,
};
...
typedef enum {
	...
	/** record aps monitor enable */
	MEDIA_PARAM_SET_RECORD_APS_MONITOR_ENABLE,
	/** get profile statistics info */
	MEDIA_PARAM_GET_PROFILE,
	/** set soft volume */
	MEDIA_PARAM_SET_SOFT_VOLUME,

	/** set lost pkt mode */
	MEDIA_PARAM_SET_LOST_PKT_MODE,
    MEDIA_PARAM_SET_LEFT_CHAN_HANDLE,
    MEDIA_PARAM_SET_RIGHT_CHAN_HANDLE,
	...
	/** audio effect operation set pre-volume */
	MEDIA_EFFECT_EXT_SET_VOLUME_PRE,
    ...
    MEDIA_EFFECT_EXT_SET_UPSTREAM_ENC_ENABLE,
    MEDIA_EFFECT_EXT_SET_UPSTREAM_AI_ENC_ENABLE,
    MEDIA_EFFECT_EXT_SET_UPSTREAM_DAE_ENABLE,
    MEDIA_EFFECT_EXT_SET_UPSTREAM_CNG_ENABLE,
    MEDIA_EFFECT_EXT_SET_DOWNSTREAM_AGC_ENABLE,
    MEDIA_EFFECT_EXT_SET_MIC_CHANNEL_SEL,
    MEDIA_EFFECT_EXT_SET_DOWNSTREAM_SPEAKER_NR_ENABLE,
    MEDIA_EFFECT_EXT_SET_DOWNSTREAM_TAIL_NR_ENABLE,
} media_param_e;
...
typedef struct {
	unsigned int avg_runtime_us; /* average run time per frame in us */
	unsigned int max_runtime_us; /* maximum run time per frame in us */
} profile_sta_info_t;
...
/* nav working mode */
#define NAV_DEC_48K_2CH_ENC_32K_1CH    (0) /* headphone paired with a dongle */
#define NAV_DEC_32K_1CH_ENC_48K_2CH    (1) /* dongle */
#define NAV_ONLY_DEC_48K_2CH           (2) /* music play paired with a phone */
#define NAV_BOTH_32K_1CH               (3) /* call or speech paired with a phone */
...
typedef struct
{
	...
	uint8_t cis_m_bn;
	uint8_t cis_m_ft;
	uint8_t codec_stream_type; /* 0-standard stream, 1-private stream */
	uint8_t codec_bit_width; /* 0-16bits, 1-24bits */
	uint8_t working_mode;
	uint8_t sync_delay_mode;
	uint8_t real_time_mode;
	uint8_t dump_crc_error_pkt;
...
} format_ext_nav_param_t;

typedef union {
	format_ext_nav_param_t nav_param;
} format_ext_param_u;
...
typedef struct {
	...
	uint8_t soft_volume_enable:1;
	uint8_t tip_mix_resample:1;
	uint8_t not_support_postprocessor:1;
	uint8_t audio_track_dis:1;
    uint8_t is_double_encoder:1;
	...
	uint8_t res_out_sample_rate; /* kHz */
	uint8_t sample_bits;
	uint8_t channels;
	uint8_t uac_upload_flag;
	uint32_t latency_us; /* latency target when dsp dacfifo directly */

	uint8_t freqadj_enable;
	uint8_t freqadj_low_freq_run;
	uint16_t freqadj_high_freq_mhz;
	uint16_t tx_offset;
	...
	io_stream_t usound_output_stream;
	...
	/* capture source, 0-don't care, 1-adc, 2-is2rx */
	uint8_t capture_source;
	uint16_t capture_bit_rate; /* kbps */
	uint16_t capture_frame_size; /* sample pairs */
	...
	/** ext param according to format */
	format_ext_param_u ext_param;
...
/**
 * @brief pre enable for media service
 *
 * This routine provides to pre enable meida service
 *
 * @param msg message of service pre enable
 ...
 */
int media_service_pre_enable(struct app_msg *msg);

/**
 * @brief pre start play for media service
 *
 * This routine provides to pre start meida service play
 *
 * @param msg message of service pre play
 *	msg.type = MSG_MEDIA_SRV_PRE_PLAY;
 *	msg.ptr = handle->media_srv_handle handle of media service ;
 *
 * @return 0 excute successed , others failed
 */
int media_service_pre_play(struct app_msg *msg);
