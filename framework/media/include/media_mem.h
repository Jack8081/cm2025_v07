...
typedef enum {
    ...
    DMVP_PARAM,
    AI_PARAM,
    DAE_PRE_PARAM, /* le audio game upstream preprocess dae param */

	/* bt controller and dsp share info */
	CIS_SHARE_INFO,
	CIS_XFER_REPORT_DATA,
	CIS_SEND_REPORT_DATA,
	SCO_XFER_REPORT_DATA,
...
/**
 * @brief get cache mem pool size for le audio stream
 *
 * This routine provides to get target memory size
 *
 * @param mem_type memory type @see cache_pool_type_e
 * @param stream_type indicator for meida sence
 * @param format NAV_TYPE
 * @param block_size nav sdu
 * @param block_num nav packet number
 *
 * @return size of memory of needed
 */
