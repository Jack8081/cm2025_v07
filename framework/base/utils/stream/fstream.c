...
static int file_name_has_cluster(char *file_name, char **dir, uint32_t *clust, uint32_t *blk_ofs);
...
int fstream_open(io_stream_t handle, stream_mode mode)
{
    ...
    if(mode & MODE_OUT) {
        fs_mode |= FA_WRITE | FA_OPEN_ALWAYS;
    }

    if (file_name_has_cluster(info->file_name, &dir, &cluster, &blk_ofs)) {
		res = fs_open_cluster(&info->fp, dir, cluster, blk_ofs, fs_mode);
		if (res) {
			SYS_LOG_ERR("open Failed %d\n", res);
			goto failed;
		}
	} else {
		res = fs_open(&info->fp, info->file_name, fs_mode);
		if (res) {
			SYS_LOG_ERR("open Failed %d\n", res);
			goto failed;
		}
...
int fstream_init(io_stream_t handle, void *param)
{
    ...
    info->file_name = mem_malloc(strlen((char *)param) + 1);
    if(NULL == info->file_name) {
        goto FAIL;
    }
    
    strcpy(info->file_name, (char *)param);
	handle->data = info;
	return 0;
