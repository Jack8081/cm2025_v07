...
void *ringbuff_stream_get_ringbuf_buf(io_stream_t handle)
{
	if (handle && handle->type == RINGBUFFER_STREAM_IMAG_NUM ) {
		ringbuff_info_t *info = (ringbuff_info_t *)handle->data;
		if (info && info->buf)
			return (void *)((info->buf)->cpu_ptr);
	} 
	return NULL;
}

uint32_t ringbuff_stream_get_ringbuf_size(io_stream_t handle)
{
	if (handle && handle->type == RINGBUFFER_STREAM_IMAG_NUM ) {
		ringbuff_info_t *info = (ringbuff_info_t *)handle->data;
		if (info && info->buf)
			return (info->buf)->size;
	} 
	return 0;
}
