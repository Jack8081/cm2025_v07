...
#define OTA_ERASE_PAGE_SIZE (4096)
...
#define IS_STORAGE_TYPE_SD_NAND(x) ((x)->storage_type == OTA_STORAGE_SD || (x)->storage_type == OTA_STORAGE_NAND)
static struct ota_storage global_ota_storage[OTA_STORAGE_MAX_TYPE];

int ota_storage_sync(struct ota_storage *storage)
{
	if (storage && storage->dev && storage->storage_type == OTA_STORAGE_NAND)
		      ...
		      uint8_t *buf, int size)
{
	...
	if (count) {
		err = flash_write(storage->dev, offs, buf, count);
		if (err) {
	...
	if (left) {
		...
		err = flash_write(storage->dev, offs, buffer, 1);
		     ...
		     uint8_t *buf, int size)
{
	...
	if (count) {
		err = flash_read(storage->dev, offs, buf, count);
		if (err) {
	...
	if (left) {
		...
		err = flash_read(storage->dev, offs, buffer, 1);
		if (err) {
			...
			uint8_t *buf, int buf_size)
{
	...
	if (IS_STORAGE_TYPE_SD_NAND(storage)) {
		//SYS_LOG_INF("sd: ignore check clean");
		return ota_storage_sd_nand_is_clean(storage, offs, size, buf, buf_size);
	}
...
int ota_storage_erase_spinor(struct ota_storage *storage, int offs, int size)
{
	...
	erase_size = (size > OTA_ERASE_PAGE_SIZE) ? OTA_ERASE_PAGE_SIZE : size;
...
int ota_storage_erase(struct ota_storage *storage, int offs, int size)
{
	...
	if (IS_STORAGE_TYPE_SD_NAND(storage)) {
		//SYS_LOG_INF("sd: ignore erase");
		return ota_storage_erase_sd_nand(storage, offs, size);
	} else if (storage->storage_type == OTA_STORAGE_SPINOR){
...
{
	if (storage_id < OTA_STORAGE_MAX_TYPE)
