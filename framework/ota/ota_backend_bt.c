...
int ota_cmd_h2d_send_image_data(struct svc_prot_context *ctx, uint16_t param_len, int with_crc)
{
	...
	/* read data */
	err = svc_prot_get_rx_data(ctx, ctx->read_buf + ctx->read_done_len, seg_len);
	if (err) {
		SYS_LOG_ERR("read error, err %d", err);
		return -EIO;
	}
	...
	if (with_crc) {
		crc = utils_crc32(0, ctx->read_buf + ctx->read_done_len, seg_len);
		if (crc != crc_orig) {
...
int tr_ota_backend_bt_read_data(struct ota_backend *backend, uint8_t *buf, int size)
{
	struct ota_backend_bt *backend_bt = CONTAINER_OF(backend,
		struct ota_backend_bt, backend);
	struct svc_prot_context *svc_ctx = &backend_bt->svc_ctx;
	...
	if (svc_ctx->sppble_stream && svc_ctx->sppble_stream_opened == 0) {
	...
	err = svc_prot_get_rx_data(svc_ctx, buf, size);
	return err;
}

int tr_ota_backend_bt_write_data(struct ota_backend *backend, uint8_t *buf, int size)
{
	struct ota_backend_bt *backend_bt = CONTAINER_OF(backend,
		struct ota_backend_bt, backend);
	struct svc_prot_context *svc_ctx = &backend_bt->svc_ctx;
	...
	err = svc_prot_send_data(svc_ctx, buf, size);
	return err;
}
#endif
...
static void sppble_connect_cb(int connected)
{
	struct ota_backend *backend;
...
#ifdef CONFIG_SPPBLE_DATA_CHAN
    .tr_read_data = tr_ota_backend_bt_read_data,
    .tr_write_data = tr_ota_backend_bt_write_data,
#endif
					...
					struct ota_backend_bt_init_param *param)
{
	...
	if (!svc_ctx->send_buf) return NULL;
