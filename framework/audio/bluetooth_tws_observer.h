...
#else
#define SDM_CNT_MASK  (0x7FFF)
#endif


#ifdef CONFIG_SOFT_SDM_CNT
#define SOFT_SDM_OSR_SHIFT  (13)
...
typedef struct {
    ...
    int32_t (*set_stream_info)(void *handle, uint8_t format, uint8_t channels,
        uint16_t first_pktseq, uint16_t sample_rate,
        uint32_t cache_time_us, uint32_t pkt_time_us, uint64_t play_time);
    ...
    int32_t (*set_sync_policy)(void *handle, uint32_t type, uint8_t enable);
} bt_tws_observer_t;
    ...
    uint32_t cache_time_us, uint32_t pkt_time_us, uint64_t play_time)
{
    if(handle->set_stream_info) {
        return handle->set_stream_info(handle, format, channels, first_pktseq, sample_rate, cache_time_us, pkt_time_us, play_time);
    } else {
        return -1;
    }
}

static inline int32_t bt_tws_set_sync_policy(bt_tws_observer_t *handle, uint32_t type, uint8_t enable)
{
    if(handle->set_sync_policy) {
        return handle->set_sync_policy(handle, type, enable);
    } else {
        return -1;
    }
}
