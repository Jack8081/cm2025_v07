...
repeat:
	...
	case OTG_STATE_B_WAIT_ACON:
		...
		if (++usb_connect_count >= USB_CONNECT_COUNT_MAX) {
			SYS_LOG_INF("connected to charger");
...
#define HOTPLUG_STABLE_MAX      (30)
#define HOTPLUG_LAZY_TIME       (30)

static int usb_hotplug_loop(void)
{
    struct app_msg msg = {0};
    static uint8_t hotplug_stable = HOTPLUG_STABLE_MAX;
    static uint8_t audio_app_bt   = AUDIO_APP_ID_LE_AUDIO;
    static uint8_t lazy_time      = HOTPLUG_LAZY_TIME;
	...
	if ((otg_state == old_state) && (hotplug_stable == 0)) {
        return 0;
	} else {
        if (otg_state != old_state) {
            ...
            hotplug_stable = HOTPLUG_STABLE_MAX;
        }
    ...
    case OTG_STATE_B_WAIT_ACON:
        ...
        /* USB connected, switch to usound application */
        if (strcmp(APP_ID_USOUND, app_manager_get_current_app())) {
            /* Update bt audio app */
            audio_app_bt  = bt_manager_get_active_app_id();
	...
	case OTG_STATE_A_IDLE:
        ...
        /* USB disconnected, switch to bt audio application (leaudio/btmusic) */
        if (audio_app_bt == AUDIO_APP_ID_BT_MUSIC) {
            if (strcmp(APP_ID_BTMUSIC, app_manager_get_current_app())) {
                msg.type  = MSG_HOTPLUG_EVENT;
                msg.cmd   = HOTPLUG_USB_DEVICE;
                msg.value = AUDIO_APP_ID_BT_MUSIC;
                send_async_msg(APP_ID_MAIN, &msg);

                hotplug_stable = HOTPLUG_STABLE_MAX;
            }
        } else if (audio_app_bt == AUDIO_APP_ID_LE_AUDIO) {
