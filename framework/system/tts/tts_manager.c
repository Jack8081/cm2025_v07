...
static io_stream_t _tts_create_stream(uint8_t *filename, bool tele_num)
{
	...
	if (tts_ctx->tts_config->tts_storage_media == TTS_SOTRAGE_SDFS) {
		...
		stream = file_stream_create(filename);
		if (!stream) {
			SYS_LOG_ERR("create failed\n");
			goto exit;
		}
...
static int _tts_start_play(struct tts_item_t *tts_item, const char *file_name)
{
	...
	init_param.not_support_postprocessor = 1;
...
#ifdef CONFIG_BT_TRANSCEIVER
#if (defined CONFIG_SAMPLE_CASE_1) || (defined CONFIG_SAMPLE_CASE_XNT)
        if(strcmp(tts_item->tts_file_name, "V_POWON.act") == 0)
        {
...
static void _tts_manager_stop_work(os_work *work)
{
	...
	dvfs_set_level(DVFS_LEVEL_HIGH_PERFORMANCE, "tts");
#endif
	...
	_tts_manager_triggle_play_tts();
#ifdef CONFIG_DVFS_DYNAMIC_LEVEL
	dvfs_unset_level(DVFS_LEVEL_HIGH_PERFORMANCE, "tts");
#endif	
}
...
#ifdef CONFIG_BT_TRANSCEIVER
	if(tts_ctx->tip_locked) {
        SYS_LOG_INF("tip locked %d\n", tts_ctx->tip_locked);
        ret = -1;
        goto exit;
	}
#endif
...
int tts_manager_init(void)
{
    ...
    tts_manager_lock(); //close dongle tts
#endif
	return 0;
}
