...
#define BT_BUF_ISO_SIZE(size) (CONFIG_BT_HCI_RESERVE + \
					  BT_HCI_ISO_HDR_SIZE + \
...
struct bt_iso_chan {
	...
	/* Channel established parameters */
	union {
		struct bt_iso_cis_params *cis_params;
		struct bt_iso_bis_params *bis_params;
	};
	uint8_t				state;
...
int bt_iso_conn_group_id(struct bt_conn *iso);

int bt_iso_conn_stream_id(struct bt_conn *iso);

struct bt_conn *bt_conn_from_iso(struct bt_conn *iso);
