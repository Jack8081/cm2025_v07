...
struct bt_hfp_hf_cb {
	 ...
	 *  @param type: Phone number type.
	 */
	void (*clip)(struct bt_conn *conn, uint8_t *buf, uint32_t type);
	 ...
	 */
	void (*clcc)(struct bt_conn *conn, uint32_t idx, uint32_t dir, uint32_t status, uint32_t mode, uint32_t mpty);
