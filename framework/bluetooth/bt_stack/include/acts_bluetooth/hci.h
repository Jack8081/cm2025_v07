...
#define bt_iso_pkt_len(h)                ((h) & 0xfff)
...
#define BT_HCI_OP_READ_LINK_QUALITY             BT_OP(BT_OGF_STATUS, 0x0003)
struct bt_hci_cp_read_link_quality {
	uint16_t handle;
} __packed;
struct bt_hci_rp_read_link_quality {
	uint8_t status;
	uint16_t handle;
	uint8_t quality;
} __packed;
...
struct bt_hci_cp_wirte_link_policy {
	uint16_t   handle;
	uint16_t   link_policy;
} __packed;
#endif
