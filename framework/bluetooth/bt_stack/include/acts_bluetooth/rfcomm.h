...
struct bt_rfcomm_dlc_ops {
     ...
     *  @param tx_credit
	 */
	void (*update_credit)(struct bt_rfcomm_dlc *dlc, u8_t tx_credit);
#endif
};
...
/** @brief Allocate the buffer from pool with length,
 *  L2CAP and ACL headers.
 *
 *  @param pool Which pool to take the buffer from.
 *
 *  @return New buffer.
 */
struct net_buf *bt_rfcomm_create_pdu_len(struct net_buf_pool *pool, uint16_t len);
