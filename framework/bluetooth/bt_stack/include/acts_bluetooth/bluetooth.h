...
#define L2CAP_BR_ACT_FIX_CID_001E	0x001E
#define L2CAP_BR_ACT_FIX_CID_001F	0x001F
...
typedef void(*complete_event_cb)(uint8_t status, uint8_t *data, uint16_t len);

struct bt_stack_config_param {
	...
	uint8_t hfp_sp_codec_neg:1;		/* Hfp support codec negotiation  */
...
#define BT_LE_SCAN_OPT_FILTER_WHITELIST BT_LE_SCAN_OPT_FILTER_ACCEPT_LIST
...
enum {
	BT_CB_EVT_VS_ACL_SNOOP_LINK_COMPLETE 		= 0xFF01,		/* FF: vendor event, 07: vendor event number */
	BT_CB_EVT_VS_SNOOP_LINK_DISCONNECT 			= 0xFF02,
	BT_CB_EVT_VS_SCO_SNOOP_LINK_COMPLETE 		= 0xFF03,
	BT_CB_EVT_VS_SNOOP_SWITCH_COMPLETE			= 0xFF04,
	BT_CB_EVT_VS_SNOOP_MODE_CHANGE				= 0xFF05,
	BT_CB_EVT_VS_TWS_MATCH_REPORT 				= 0xFF07,
	BT_CB_EVT_VS_SNOOP_ACTIVE_LINK 				= 0xFF08,
	BT_CB_EVT_VS_SYNC_1ST_PKT_CHG				= 0xFF09,
	BT_CB_EVT_VS_TWS_SNOOP_EXIT_SNIFF			= 0xFF0A,
};

typedef void (*bt_evt_cb_func_t)(uint16_t event, void *param);

struct bt_cb_evt_vs_snoop_link_param {
	struct bt_conn *conn;
	uint32_t com_param;			/* common parameter */
	bt_addr_t *peer_bdaddr;		/* For link complete */
	bt_addr_t *local_bdaddr;	/* For acl link complete */
	uint8_t *link_key;			/* For acl link complete */
	uint8_t link_type;			/* For sync link complete */
	uint8_t codec_id;			/* For sync link complete */
	uint16_t packet_len;		/* For sync link complete */
};

/** @brief register evt callback function
 *
...
/** @brief Initialize bt stack config parameter
 *
...
#define TWS_MATCH_NAME_MAX_LEN   26

enum {
	TWS_MATCH_MODE_NAME = 0,  /* Tws pair match by name */
	TWS_MATCH_MODE_ID   = 1,  /* Tws pair match by device id */
};

struct tws_visual_mode_param {
	uint8_t enable;
	uint16_t interval;
...
int bt_vs_set_set_le_audio_user_param(int *param, int array_size);

int bt_stack_sync_info_len(void);
bool bt_stack_ready_sync_info(struct bt_conn *conn);
void *bt_stack_get_sync_info(struct bt_conn *conn, void *info);
struct bt_conn *bt_stack_set_sync_info(void *info, uint16_t len);
int bt_le_stack_sync_info_len(void);
bool bt_le_stack_ready_sync_info(struct bt_conn *conn);
void *bt_le_stack_get_sync_info(struct bt_conn *conn, void *info);
struct bt_conn *bt_le_stack_set_sync_info(void *info, uint16_t len);
void bt_stack_dump_info(void);
