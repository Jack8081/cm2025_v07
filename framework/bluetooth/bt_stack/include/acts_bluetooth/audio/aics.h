...
#define BT_AICS_PRIMARY_MICS 0
#define BT_AICS_PRIMARY_VCS  1
...
typedef void *(*bt_aics_lookup_cb)(struct bt_conn *conn,
...
int bt_aics_gain_mode_set(struct bt_aics *inst, uint8_t mode);

void bt_aics_server_cb_register(bt_aics_lookup_cb cb);

ssize_t bt_aics_read_state(struct bt_conn *conn,
			       const struct bt_gatt_attr *attr, void *buf,
			       uint16_t len, uint16_t offset);

ssize_t bt_aics_read_gain_settings(struct bt_conn *conn,
				       const struct bt_gatt_attr *attr,
				       void *buf, uint16_t len, uint16_t offset);

ssize_t bt_aics_read_type(struct bt_conn *conn, const struct bt_gatt_attr *attr,
			 void *buf, uint16_t len, uint16_t offset);

ssize_t bt_aics_read_input_status(struct bt_conn *conn,
				 const struct bt_gatt_attr *attr, void *buf,
				 uint16_t len, uint16_t offset);

ssize_t bt_aics_write_control(struct bt_conn *conn,
				  const struct bt_gatt_attr *attr, const void *buf, uint16_t len,
				  uint16_t offset, uint8_t flags);

ssize_t bt_aics_write_description(struct bt_conn *conn,
				 const struct bt_gatt_attr *attr,
				 const void *buf, uint16_t len, uint16_t offset,
				 uint8_t flags);

ssize_t bt_aics_read_description(struct bt_conn *conn,
				const struct bt_gatt_attr *attr, void *buf,
				uint16_t len, uint16_t offset);
