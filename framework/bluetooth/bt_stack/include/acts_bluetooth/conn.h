...
enum {
	...
	BT_CONN_TYPE_BR_SNOOP = BIT(4),
	/** SCO SNOOP Connection Type */
	BT_CONN_TYPE_SCO_SNOOP = BIT(5),
	/** All Connection Type */
	BT_CONN_TYPE_ALL = BT_CONN_TYPE_LE | BT_CONN_TYPE_BR |
			   BT_CONN_TYPE_SCO | BT_CONN_TYPE_ISO | BT_CONN_TYPE_BR_SNOOP |
			   BT_CONN_TYPE_SCO_SNOOP,
};
...
int bt_conn_get_info_by_handle(uint16_t handle, struct bt_conn_info *info);
...
struct bt_conn_cb {
	 ...
	 */
	void (*remote_linkkey_miss)(bt_addr_t *peer);
...
//#endif /* defined(CONFIG_BT_SMP) || defined(CONFIG_BT_BREDR) */
	 ...
	 *	@param len Data length.
	 */
	void (*rx_act_fix_cid_data)(struct bt_conn *conn, uint16_t cid, uint8_t *data, uint16_t len);
 ...
 *  @return  Zero for success, non-zero otherwise.
 */
int bt_conn_send_act_fix_cid_data(struct bt_conn *conn, uint16_t cid, uint8_t *data, uint16_t len);

int bt_conn_send_act_fix_cid_data_timeout(struct bt_conn *conn, uint16_t cid, uint8_t *data, uint16_t len, k_timeout_t timeout);
...
uint8_t bt_conn_get_role(const struct bt_conn *conn);
 ...
 *  @return connection acl handle.
 */
uint16_t bt_conn_get_acl_handle(const struct bt_conn *conn);
 ...
 *  @return connection type.
 */
uint8_t bt_conn_get_type_by_handle(uint16_t handle);
 ...
 */
uint8_t bt_conn_get_role_by_handle(uint16_t handle);
 ...
 *  @return 0: sucess, other:  error.
 */
int bt_conn_set_snoop_media_rx_cid(struct bt_conn *conn, uint16_t cid);
 ...
 *  @return 0: sucess, other:  error.
 */
int bt_conn_set_snoop_switch_lock(struct bt_conn *conn, uint8_t lock);

/** @brief Check is le switch connected
 *
 *  @param conn Connection object.
 *
 *  @return 0: Normal le connected, other:  le switch connected.
 */
