...
static char *connect_type_to_str(uint8_t type)
{
	switch (type) {
	case BT_CONN_TYPE_LE:
		return "LE";
...
static void conn_connected(struct bt_conn *conn, uint8_t err)
{
	...
	if (type != BT_CONN_TYPE_BR && type !=  BT_CONN_TYPE_BR_SNOOP &&
		type != BT_CONN_TYPE_SCO && type != BT_CONN_TYPE_SCO_SNOOP) {
		return;
	}
	...
	LOG_INF("%s connected: %s, type %d", connect_type_to_str(type), addr, type);
	...
	case BT_CONN_TYPE_SCO_SNOOP:
		...
		if (brdev && !brdev->sco_conn) {
			brdev->sco_conn = hostif_bt_conn_ref(conn);
		} else {
			LOG_WRN("%p Had connect one sco", brdev);
		}
		break;
...
static void conn_disconnected(struct bt_conn *conn, uint8_t reason)
{
	...
	LOG_INF("%s disconnected: %s (reason %u), type %d", connect_type_to_str(type), addr, reason, type);
...
static void conn_rx_act_fix_cid_data(struct bt_conn *conn, uint16_t cid, uint8_t *data, uint16_t len)
{
	LOG_INF("Rx act fix cid 0x%x data len %d", cid, len);
	/* Test sync bt stack info */
	if (data[0] == 0x55 && data[1] == 0xAA) {
		hostif_bt_stack_set_sync_info(data, len);
	}
}
...
static struct bt_conn_cb conn_callbacks = {
	...
	.rx_act_fix_cid_data = conn_rx_act_fix_cid_data,
};
...
void spp_connected_cb(struct bt_conn *conn, uint8_t spp_id)
{
	...
	if (brdev->spp_id[0] == spp_id || brdev->spp_id[1] == spp_id) {
		...
		LOG_INF("Passive connect spp_id %d", spp_id);
...
static int cmd_spp_connect(const struct shell *shell, size_t argc, char *argv[])
{
	...
	if (spp_id) {
		...
		LOG_INF("Start connect spp spp_id %d", spp_id);
	} else {
...
void test_pbap_client_recv_cb(struct bt_conn *conn, uint8_t user_id, struct parse_vcard_result *result, uint8_t array_size)
{
	...
	for (i = 0; i < array_size; i++) {
		LOG_INF("Type %d, len %d, value %s", result[i].type, result[i].len, result[i].data);
	}
}
...
static int cmd_pbap_get(const struct shell *shell, size_t argc, char *argv[])
{
	...
	if (argc < 2) {
		...
		LOG_INF("Get PBAP path %s", argv[1]);
...
static void hid_event_handler_cb(struct bt_conn *conn, uint8_t event, uint8_t *data, uint16_t len)
{
	...
	if(event == BT_HID_EVENT_GET_REPORT){
		struct hid_stack_report *report = (struct hid_stack_report*)data;
...
void bt_evt_cb_func(uint16_t event, void *param)
{
	uint8_t type;
	char br_addr[BT_ADDR_STR_LEN];
	struct bt_cb_evt_vs_snoop_link_param *l_param = param;
	struct br_dev_manager *brdev;

	switch (event) {
	case BT_CB_EVT_VS_ACL_SNOOP_LINK_COMPLETE:
		LOG_INF("ACL_SNOOP_LINK_COMPLETE Handle 0x%x", hostif_bt_conn_get_handle(l_param->conn));
		hostif_bt_addr_to_str(l_param->peer_bdaddr, br_addr, sizeof(br_addr));
		LOG_INF("peer_bdaddr %s", br_addr);
		hostif_bt_addr_to_str(l_param->local_bdaddr, br_addr, sizeof(br_addr));
		LOG_INF("local_bdaddr %s", br_addr);
		break;
	case BT_CB_EVT_VS_SNOOP_LINK_DISCONNECT:
		LOG_INF("Snoop link disconnect type 0x%x reason %d", hostif_bt_conn_get_type(l_param->conn), l_param->com_param);
		break;
	case BT_CB_EVT_VS_SCO_SNOOP_LINK_COMPLETE:
		LOG_INF("SCO_SNOOP_LINK_COMPLETE Handle 0x%x", hostif_bt_conn_get_handle(l_param->conn));
		hostif_bt_addr_to_str(l_param->peer_bdaddr, br_addr, sizeof(br_addr));
		LOG_INF("peer_bdaddr %s", br_addr);
		LOG_INF("link_type %d, packet_len %d, codec_id %d", l_param->link_type, l_param->packet_len, l_param->codec_id);
		break;
	case BT_CB_EVT_VS_SNOOP_SWITCH_COMPLETE:
		type = hostif_bt_conn_get_type(l_param->conn);
		LOG_INF("SNOOP_SWITCH Handle 0x%x type 0x%x", hostif_bt_conn_get_handle(l_param->conn), type);
		brdev = find_brdev_by_brconn(l_param->conn);
		if (brdev) {
			brdev->type = type;
		}
		break;
	case BT_CB_EVT_VS_SNOOP_MODE_CHANGE:
		LOG_INF("Snoop(0x%x) mode change %d", hostif_bt_conn_get_handle(l_param->conn), l_param->com_param);
		brdev = find_brdev_by_brconn(l_param->conn);
		if (brdev) {
			brdev->snoop_mode = (uint8_t)l_param->com_param;
		}
		break;
	case BT_CB_EVT_VS_TWS_MATCH_REPORT:
		hostif_bt_addr_to_str(param, br_addr, sizeof(br_addr));
		LOG_INF("Tws match %s", br_addr);
		break;
	case BT_CB_EVT_VS_SNOOP_ACTIVE_LINK:
		LOG_INF("Snoop active link 0x%x", hostif_bt_conn_get_handle(l_param->conn));
		break;
	case BT_CB_EVT_VS_SYNC_1ST_PKT_CHG:
		LOG_INF("Sync 1st pkt chg 0x%x %d", hostif_bt_conn_get_handle(l_param->conn), l_param->com_param);
		break;
	case BT_CB_EVT_VS_TWS_SNOOP_EXIT_SNIFF:
		LOG_INF("Tws snoop exit sniff");
		break;
	default:
		LOG_INF("Wait todo bt evt 0x%04x", event);
		break;
	}
}
...
static void bt_ready(int err)
{
	...
	hostif_bt_reg_evt_cb_func(bt_evt_cb_func);
	bt_init_ready = 1;
}
...
static int cmd_bredr_start(const struct shell *shell, size_t argc, char *argv[])
{
	...
	hostif_bt_br_write_scan_enable(br_scan_value,NULL);
	k_sleep(K_MSEC(100));
...
static int cmd_bredr_discoverable(const struct shell *shell, size_t argc, char *argv[])
{
	...
	if (!strcmp(action, "on")) {
		...
		err = hostif_bt_br_write_scan_enable(br_scan_value,NULL);
...
static int cmd_br_info(const struct shell *shell, size_t argc, char *argv[])
{
	int i;
	char addr[BT_ADDR_LE_STR_LEN];
	...
	for (i = 0; i < CONFIG_BT_MAX_CONN; i++) {
		if (brdev_info[i].brle_conn) {
			...
			LOG_INF("index[%d] type[0x%x] handle[0x%x] addr[%s] sco[0x%x] hf[%d] ag[%d] a2dp[%d] avrcp[%d] spp[%d %d] hid[%d]",
				i, brdev_info[i].type, hostif_bt_conn_get_handle(brdev_info[i].brle_conn),
				addr, hostif_bt_conn_get_handle(brdev_info[i].sco_conn),
...
/* For test tws irq */
static void test_bt_tws0_isr(uint32_t bt_clk, uint16_t intra_off)
{
...
static int cmd_tws_irq(const struct shell *shell, size_t argc, char *argv[])
{
	...
	btdrv_tws_irq_cb_set(BT_TWS_0, test_bt_tws0_isr);
	btdrv_tws_irq_disable(BT_TWS_0);
...
static int cmd_snoop_tws(const struct shell *shell, size_t argc, char *argv[])
{
	struct bt_conn *conn = brdev_info[curr_brdev_index].brle_conn;
	struct bt_conn *sec_conn;
	struct bt_conn *sco_conn;
	struct br_dev_manager *sec_brdev;
	struct bt_conn_info info;
	const char *action;
	struct tws_visual_mode_param visual_param;
	struct tws_pair_mode_param pair_param;
	int ret, value = 0, value2 = 0;
	unsigned long ulValue;
	void *sync_info;

	/* Set second conn, for test, set conn for tws conn,  second conn for phone */
	if (curr_brdev_index == 0) {
		sec_brdev = &brdev_info[1];
		sec_conn = brdev_info[1].brle_conn;
		sco_conn = brdev_info[1].sco_conn;
	} else {
		sec_brdev = &brdev_info[0];
		sec_conn = brdev_info[0].brle_conn;
		sco_conn = brdev_info[0].sco_conn;
	}

	if (argc < 2) {
		LOG_INF("Used: ts snoop cmd xx");
		return -EINVAL;
	}

	action = argv[1];
	LOG_INF("Cmd %s", action);

	if (!strcmp(action, "visual")) {
		if (argc > 2) {
			value = atoi(argv[2]);
		}
		memset(&visual_param, 0, sizeof(visual_param));
		if (value) {
			visual_param.enable = 1;
		}
		visual_param.interval = 20*1000/625;		/* 20ms */
		visual_param.connected_phone = 0;
		visual_param.channel = 0;
		visual_param.match_mode = TWS_MATCH_MODE_NAME;
		visual_param.pair_keyword = 0;
		hostif_bt_vs_set_tws_visual_mode(&visual_param);
	} else if (!strcmp(action, "pair")) {
		if (argc > 2) {
			value = atoi(argv[2]);
		}
		...
		if (value) {
			pair_param.enable = 1;
			pair_param.setup_link = 1;
		}
		pair_param.interval = 2650*1000/625;		/* 2650ms */
		pair_param.window = 100*1000/625;			/* 100ms */
		pair_param.rssi_thres = 0x88;
		pair_param.mask_addr = 0;
		pair_param.connected_phone = 0;
		pair_param.mask_phone = 1;
		pair_param.channel = 0;
		pair_param.mask_channel = 1;
		pair_param.match_mode = TWS_MATCH_MODE_NAME;
		pair_param.mask_mode = 1;
		pair_param.pair_keyword = 0;
		pair_param.mask_keyword = 1;
		hostif_bt_vs_set_tws_pair_mode(&pair_param);
	} else if (!strcmp(action, "set_link")) {
		hostif_bt_vs_set_tws_link(conn);
	} else if (!strcmp(action, "set_addr")) {
		hostif_bt_conn_get_info(conn, &info);
		hostif_bt_vs_write_bt_addr((bt_addr_t *)info.br.dst);
	} else if (!strcmp(action, "create_acl")) {
		hostif_bt_vs_create_acl_snoop_link(sec_conn);
	} else if (!strcmp(action, "dis_acl")) {
		hostif_bt_vs_disconnect_acl_snoop_link(sec_conn);
	} else if (!strcmp(action, "create_sco")) {
		hostif_bt_vs_create_sync_snoop_link(sco_conn);
	} else if (!strcmp(action, "link_active")) {
		hostif_bt_vs_set_snoop_link_active(sec_conn);
	} else if (!strcmp(action, "sync_btclk")) {
		if (argc > 2) {
			value = atoi(argv[2]);		/* 0: Rx, 1: Send */
		}
		if (argc > 3) {
			value2 = atoi(argv[3]);		/* 0: Disable, 1: Enable */
		}
		hostif_bt_vs_set_1st_sync_btclk(value, sec_conn, value2);
	} else if (!strcmp(action, "get_cid")) {
		/* Get a2dp media rx channel id*/
		LOG_INF("A2dp media rx cid 0x%x", hostif_bt_a2dp_get_media_rx_cid(sec_conn));
	} else if (!strcmp(action, "set_cid")) {
		/* Set a2dp media rx channel id to conn*/
		if (argc <= 2) {
			LOG_INF("Used: tx snoop set_cid xxxx");
			return 0;
		}

		ulValue = strtoul(argv[2], NULL, 16);
		ret = hostif_bt_conn_set_snoop_media_rx_cid(sec_conn, (uint16_t)ulValue);
		LOG_INF("set_snoop_media_rx_cid 0x%x ret %d", (uint16_t)ulValue, ret);
	} else if (!strcmp(action, "switch")) {
		hostif_bt_conn_check_exit_sniff(sec_conn);
		for (value = 0; value < 300; value++) {
			if (sec_brdev->snoop_mode) {
				value = 300;
			}
			k_sleep(K_MSEC(10));
		}

		if (sec_brdev->snoop_mode == 0) {
			LOG_INF("Snoop mode not active");
			return  0;
		}

		if (!hostif_bt_stack_ready_sync_info(sec_conn)) {
			LOG_INF("Not ready sync info!");
			return 0;
		}

		static char info_buff[256];

		memset(info_buff, 0, sizeof(info_buff));
		sync_info = hostif_bt_stack_get_sync_info(sec_conn, info_buff);
		if (sync_info) {
			hostif_bt_conn_send_act_fix_cid_data(conn, L2CAP_BR_ACT_FIX_CID_001F, sync_info, hostif_bt_stack_sync_info_len());
			hostif_bt_vs_switch_snoop_link(sec_conn);
		}
	} else {
		LOG_INF("Unknow command");
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(acts_bt_cmds,
	SHELL_CMD(info, NULL, "Connect infomation", cmd_br_info),
	...
	SHELL_CMD(disconnect, NULL, "Disconnect", cmd_disconnect_bredr),
	...
	SHELL_CMD(tws_irq, NULL, "Test tws irq", cmd_tws_irq),
	SHELL_CMD(snoop, NULL, "Test snoop tws", cmd_snoop_tws),
