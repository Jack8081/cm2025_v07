...
struct net_buf *bt_l2cap_create_pdu_timeout_len(struct net_buf_pool *pool,
					    size_t reserve, size_t len,
					    k_timeout_t timeout);

#define bt_l2cap_create_pdu_len(_pool, _reserve, len) \
	bt_l2cap_create_pdu_timeout_len(_pool, _reserve, len, K_FOREVER)
...
typedef void(*br_snoop_media_data_cb_t)(struct bt_conn *conn, struct net_buf *buf);
int bt_l2cap_br_reg_snoop_media_data_cb_func(br_snoop_media_data_cb_t cb);

int bt_l2cap_br_set_sync_info_create(struct bt_l2cap_br_chan *br_chan);
void bt_l2cap_br_dump_info(void);

/* Actions add end */
#ifdef CONFIG_BT_TRANSCEIVER
#include "tr_l2cap_internal.h"
#endif
