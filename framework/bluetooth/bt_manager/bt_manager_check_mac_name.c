...
static uint32_t mic_gen_rand(void)
{
	...
	handle = audio_record_create(AUDIO_STREAM_VOICE, AUDIO_STREAM_DEFAULT, 16, 16, AUDIO_FORMAT_PCM_16_BIT, AUDIO_MODE_MONO, NULL);
	if (!handle) {
...
uint32_t hardware_randomizer_gen_rand(void)
{
	...
	se_trng_rand(&trng_low, &trng_high);
...
{
#ifdef CONFIG_PROPERTY
	uint8_t mac[6], i, used_random;
	int ret;
	uint32_t rand_val;

	ret = property_get(CFG_BT_MAC, mac_str, (MAC_STR_LEN-1));
	if (ret < (MAC_STR_LEN-1)) {
		used_random = bt_manager_config_get_gen_mac_method(mac);
		if (used_random) {
...
int bt_manager_set_leaudio_name(uint8_t *leaudio_name)
{
#ifdef CONFIG_PROPERTY
	uint8_t len = 0;
	uint8_t *cfg_name = CFG_BLE_AUDIO;
	uint8_t lea_name[BT_NAME_LEN];
	int ret;
	...
	memset(lea_name, 0, sizeof(lea_name));
	
	ret = property_get(cfg_name, lea_name, (BT_NAME_LEN-1));
	if(ret < 0){
		len = strlen(leaudio_name);
		if (len > (BT_NAME_LEN-1)){
			SYS_LOG_ERR("name too long");
			return -1;
		}
		...
		if (len > 0) {
			property_set_factory(cfg_name, leaudio_name, len);
		}
	}

	SYS_LOG_INF("%s: %s", cfg_name, leaudio_name);
#endif
	return 0;
}
