...
void bt_manager_check_link_status(void)
{
    ...
    if (bt_manager->tws_pair_search){
        link_status = BT_STATUS_TWS_PAIR_SEARCH;
    }
    else if((bt_manager->connected_phone_num > 0) || bt_manager_audio_lea_addr()){
    ...
    else
    {
        switch(bt_manager->pair_mode_state){
            ...
            case BT_PAIR_MODE_DISCONNECT_PHONE:
                ...
                if (cfg_feature->sp_dual_phone == 0 && phone_num > 0)
                {
                    disc_mode = 1;						
                }
				...
				if(disc_mode){
					if(disc_mode == 1){
					    ...
					    bt_manager_disconnect_nonactive_dev();
					}
					bt_manager->pair_mode_state = BT_PAIR_MODE_CHECK_PHONE;
				}
				else{
