...
static void _bt_manager_irq_work(os_work *work)
{
	...
	if (!sys_slist_is_empty(&tws_context.tws_sync_event_list)) {
		SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&tws_context.tws_sync_event_list, event_item, temp, node) {
			if (event_item && event_item->bt_clock <= bt_manager_tws_get_bt_clock()) {
...
static int _bt_manager_tws_event_notify(int event_type, void *param, int param_size)
{
	...
	case BTSRV_TWS_DISCONNECTED:
		...
		audiolcy_set_latency_mode(tws_context.record_low_latency, 1);
	...
	case BTSRV_TWS_UPDATE_PEER_VERSION:
		...
		tws_context.record_low_latency = audiolcy_get_latency_mode();
		if (bt_manager_tws_check_support_feature(TWS_FEATURE_LOW_LATENCY)) {
			audiolcy_set_latency_mode(tws_context.record_low_latency, 1);
		} else {
			audiolcy_set_latency_mode(0, 1);
		}
		break;
...
void bt_manager_tws_send_event_sync(uint8_t event, uint32_t event_param)
{
	...
	uint32_t bt_clock = bt_manager_tws_get_bt_clock() + TWS_TIMETO_BT_CLOCK(100);
...
static void bt_manager_tws_to_us281b_send_command(uint8_t *command, int command_len)
{
	uint8_t cmd[8], event;
	uint32_t param;
...
int bt_manager_tws_send_command(uint8_t *command, int command_len)
{
	...
	return command_len;
#else
	return btif_tws_send_command(command, command_len);
#endif
}
...
#else
	return btif_tws_send_command_sync(command, command_len);
#endif
}
...
void bt_manager_tws_pair_search(void)
{
	...
	if ((bt_manager_get_connected_dev_num() < CFG_BT_MANAGER_MAX_BR_NUM) &&
		btif_tws_can_do_pair()) {
		bt_manager_set_status(BT_STATUS_MASTER_WAIT_PAIR);
		 btif_tws_start_pair(TWS_PAIR_TYR_TIMES);
	} else {
		btif_tws_end_pair();
	}
}
