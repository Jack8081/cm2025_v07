...
static void btmgr_tws_disconnected_cb(uint8_t *reason)
{
	...
	if (0x13 == bt_manager->tws_dis_reason)//remote user term conn
	{
		if ((bt_manager->local_req_poweroff == 1) ||
			(bt_manager->remote_req_poweroff && (bt_manager->single_poweroff == 0))){
			SYS_LOG_INF("IN_POWOFF");
		}else{
...
void bt_manager_tws_set_pair_addr_match(bool enable)
{
    btif_tws_set_pair_addr_match(enable);
}
...
void bt_manager_tws_end_pair_search(void)
{
    ...
    SYS_LOG_INF("search:%d", bt_manager_is_tws_pair_search());

	bt_manager->tws_pair_search = 0;
    ...
    if ((cfg_tws_pair->enable_advanced_pair_mode) || (bt_manager_tws_get_pair_keyword() != 0)){
        ...
        bt_manager_tws_set_pair_addr_match(false);
    }
    else{
...
void bt_manager_tws_start_pair_search_ex(u8_t tws_pair_keyword)
{
	...
	bt_manager->tws_pair_search = 1;

    bt_manager_set_status(BT_STATUS_TWS_PAIR_SEARCH);
}
...
static int btmgr_tws_event_notify(int event_type, void *param, int param_size)
{
	...
	case BTSRV_TWS_DISCONNECTED:
		...
		btmgr_tws_update_ble_state(tws_context.tws_role);
		bt_manager_audio_tws_disconnected();
		break;
...
bool bt_manager_tws_powon_auto_pair(void)
{
    ...
    if (tws_pair_config->power_on_auto_pair_search){
        info = bt_mem_malloc(sizeof(struct autoconn_info)* MAX_MGR_DEV);
        if (info == NULL) {
            SYS_LOG_ERR("malloc failed");
            return result;
        }
...
void bt_manager_tws_init(void)
{
	...
	pair_param.interval = 250;
	pair_param.window = 100;
...
{
	tws_timer_info_t* timer_info = (tws_timer_info_t*)bt_mem_malloc(sizeof(tws_timer_info_t));
	uint32_t curr_bt_clock;

	if (!timer_info)
	{
		SYS_LOG_ERR("malloc fail");
		bt_mem_free(timer_param);
		return -1;
	}
...
static void tws_timer_work_proc(os_work *work)
{
	...
	list_for_each_entry_safe(timer_info, ttimer_info, &tws_timer_list, node)
	{
		...
		if((timer_info->bt_clock > bt_clock) && (abs(diff_time) > 2))
		{
