...
extern void hci_data_log_enable(bool enable);
...
static uint8_t test_value_cnt = 0;
...
static void test_spp_receive_data_cb(uint8_t channel, uint8_t *data, uint32_t len)
{
	...
	static uint32_t pre_time;
	static uint32_t count;
	int i, continue_flag = 0, start_flag = 0;

	if (data[0] != test_value_cnt) {
		start_flag = 1;
		test_value_cnt = data[0];
	}

	for (i=0; i<len; i++) {
		if (data[i] != test_value_cnt) {
			continue_flag = 1;
		}
	...
	if (start_flag || continue_flag) {
		printk("Rx diff flag %d %d\n", start_flag, continue_flag);
	}
...
#define SPEED_BLE_SYNC_INFO_ID		(0xFFC0)
...
struct ble_test_info {
	uint8_t connected:1;
	uint8_t speed_tx:1;
	uint8_t notify_enable:1;
	uint8_t switch_pending_send:1;
	uint8_t sync_info_flag:1;
	uint8_t sync_notify_enable:1;
	uint8_t sync_speed_tx:1;
	os_delayed_work delay_work;
	uint8_t send_buf[BLE_TEST_SEND_SIZE];
	bt_addr_t* bt_addr;	
};

static const char ble_speed_tx_trigger[] = "1122334455";
static struct ble_test_info s_ble_test;
...
{
	SYS_LOG_INF("value: %d\n", value);
	s_ble_test.notify_enable = value ? 1 : 0;
}
...
static void ble_speed_test_delaywork(os_work *work)
{
	...
	if (!s_ble_test.connected) {
		return;
	}

	if (s_ble_test.speed_tx && s_ble_test.notify_enable &&
		(s_ble_test.switch_pending_send == 0)) {
		mtu = bt_manager_get_ble_mtu(s_ble_test.bt_addr) - 3;
		...
		for (j = 0; j < repeat; j++) {
			for (i = 0; i < mtu; i++) {
				s_ble_test.send_buf[i] = data++;
			}

			ret = bt_manager_ble_send_data(s_ble_test.bt_addr, &ble_speed_attrs[3], &ble_speed_attrs[4], s_ble_test.send_buf, mtu);
			if (ret < 0) {
				break;
			}
	...
	os_delayed_work_submit(&s_ble_test.delay_work, BLE_TEST_WORK_INTERVAL);
}

static void test_ble_speed_start_stop_delaywork(void)
{
	if (!s_ble_test.speed_tx) {
		s_ble_test.speed_tx = 1;
		os_delayed_work_submit(&s_ble_test.delay_work, BLE_TEST_WORK_INTERVAL);
		SYS_LOG_INF("BLE tx start\n");
	} else {
		s_ble_test.speed_tx = 0;
		os_delayed_work_cancel(&s_ble_test.delay_work);
...
static void ble_speed_connect_cb(uint8_t *mac, uint8_t connected)
{
	...
	if (connected) {
		if (!s_ble_test.bt_addr)
		{
			s_ble_test.bt_addr = mem_malloc(sizeof(bt_addr_t));
			memcpy(s_ble_test.bt_addr, mac, sizeof(bt_addr_t));
		}

		s_ble_test.connected = 1;
		if (s_ble_test.sync_info_flag) {
			s_ble_test.speed_tx = s_ble_test.sync_speed_tx;
			s_ble_test.notify_enable = s_ble_test.sync_notify_enable;
			s_ble_test.sync_info_flag = 0;

			if (s_ble_test.speed_tx) {
				s_ble_test.switch_pending_send = 1;
				os_delayed_work_submit(&s_ble_test.delay_work, BLE_TEST_WORK_INTERVAL);
			}
		}
		hci_data_log_enable(false);
		...
		if (s_ble_test.bt_addr){
			mem_free(s_ble_test.bt_addr);
			s_ble_test.bt_addr = NULL;	
		}
	
		s_ble_test.connected = 0;
		hci_data_log_enable(true);
		s_ble_test.speed_tx = 0;
		s_ble_test.notify_enable = 0;
	}
}
...
static struct ble_reg_manager ble_speed_mgr = {
	...
	.gatt_svc = {
		.attrs = ble_speed_attrs,
		.attr_count = ARRAY_SIZE(ble_speed_attrs),
	},
};

struct blemgr_test_sync_info {
	uint8_t id;					/* Snoop switch cb func id */
	uint16_t sync_uuid;
	uint8_t notify_enable:1;
	uint8_t speed_tx:1;
};

static void ble_speed_start_switch_cb(uint8_t id)
{
	struct blemgr_test_sync_info info;

	s_ble_test.switch_pending_send = 1;

	memset(&info, 0, sizeof(info));
	info.id = BT_SWITCH_CB_ID_TEST_SAMPLE;
	info.sync_uuid = SPEED_BLE_SYNC_INFO_ID;
	info.notify_enable = s_ble_test.notify_enable;
	info.speed_tx = s_ble_test.speed_tx;
	bt_manager_tws_send_snoop_switch_sync_info((uint8_t *)&info, sizeof(info));
}

static int ble_speed_check_ready_switch_cb(uint8_t id)
{
	return 1;
}

static void ble_speed_switch_finish_cb(uint8_t id, uint8_t is_master)
{
	s_ble_test.switch_pending_send = 0;
	SYS_LOG_INF("Switch finish new role %d", is_master);
}

static void ble_speed_rx_sync_info_cb(uint8_t id, uint8_t *data, uint16_t len)
{
	struct blemgr_test_sync_info info;

	memcpy(&info, data, sizeof(info));
	if (info.sync_uuid == SPEED_BLE_SYNC_INFO_ID) {
		SYS_LOG_INF("Rx sync info");
		if (s_ble_test.connected) {
			s_ble_test.notify_enable = info.notify_enable;
			s_ble_test.speed_tx = info.speed_tx;
		} else {
			s_ble_test.sync_info_flag = 1;
			s_ble_test.sync_notify_enable = info.notify_enable;
			s_ble_test.sync_speed_tx = info.speed_tx;
		}
	}
}

static const struct tws_snoop_switch_cb_func ble_test_snoop_cb = {
	.id = BT_SWITCH_CB_ID_TEST_SAMPLE,
	.start_cb = ble_speed_start_switch_cb,
	.check_ready_cb = ble_speed_check_ready_switch_cb,
	.finish_cb = ble_speed_switch_finish_cb,
	.sync_info_cb = ble_speed_rx_sync_info_cb,
};
...
static int shell_cmd_btble_reg(const struct shell *shell, size_t argc, char *argv[])
{
	...
	if (reg_flag) {
		...
		bt_manager_tws_reg_snoop_switch_cb_func((struct tws_snoop_switch_cb_func *)&ble_test_snoop_cb, true);
		os_delayed_work_init(&s_ble_test.delay_work, ble_speed_test_delaywork);
		reg_flag = 1;
...
static int shell_cmd_btble_send(const struct shell *shell, size_t argc, char *argv[])
{
	...
	if (s_ble_test.notify_enable && (s_ble_test.switch_pending_send == 0)) {
		mtu = bt_manager_get_ble_mtu(s_ble_test.bt_addr) - 3;
		mtu = (mtu > BLE_TEST_SEND_SIZE) ? BLE_TEST_SEND_SIZE : mtu;
		...
		bt_manager_ble_send_data(s_ble_test.bt_addr, &ble_speed_attrs[3], &ble_speed_attrs[4], s_ble_test.send_buf, mtu);
	} else {
...
static void test_le_scan_result_cb(uint8_t addr_type, uint8_t *addr, int8_t rssi,
										uint8_t adv_type, uint8_t *data, uint8_t data_len)
{
	SYS_LOG_INF("addr_type %d addr %02x:%02x:%02x:%02x:%02x:%02x rssi %d",
				addr_type, addr[0], addr[1], addr[2], addr[3], addr[4], addr[5], rssi);
	SYS_LOG_INF("adv_type %d data 0x%x len %d", adv_type, data[0], data_len);
}

static int shell_cmd_btble_scan_start(const struct shell *shell, size_t argc, char *argv[])
{
#if 0	/* Ble scan with whitelist */
	uint8_t whilelist_addr[] = {0x69, 0xa9, 0xfc, 0xfd, 0x4e, 0xf4};

	bt_manager_ble_clear_whitelist();
	bt_manager_ble_add_whitelist(BT_ADDR_LE_PUBLIC, whilelist_addr);
	bt_manager_ble_start_scan(BT_LE_SCAN_TYPE_PASSIVE, 0x00, BT_LE_SCAN_OPT_FILTER_WHITELIST,
								100, 20, test_le_scan_result_cb);
#else
	bt_manager_ble_start_scan(BT_LE_SCAN_TYPE_PASSIVE, 0x00, BT_LE_SCAN_OPT_NONE,
								100, 20, test_le_scan_result_cb);
#endif
	SYS_LOG_INF("Ble scan start");
	return 0;
}

static int shell_cmd_btble_scan_stop(const struct shell *shell, size_t argc, char *argv[])
{
	bt_manager_ble_stop_scan();
	SYS_LOG_INF("Ble scan stop");
	return 0;
}
#endif
...
static struct bt_gatt_attr ota_gatt_attr[] = {
	...
	BT_GATT_CHARACTERISTIC(BLE_OTA_CHA_TX_UUID, BT_GATT_CHRC_NOTIFY,
							BT_GATT_PERM_READ | BT_GATT_PERM_WRITE, NULL, NULL, NULL),
...
SHELL_STATIC_SUBCMD_SET_CREATE(bt_mgr_cmds,
	...
	SHELL_CMD(ble_scan_start, NULL, "BLE start scan", shell_cmd_btble_scan_start),
	SHELL_CMD(ble_scan_stop, NULL, "BLE stop scan", shell_cmd_btble_scan_stop),
#endif
#if MGR_SPPBLE_STREAM_TEST_SHELL
