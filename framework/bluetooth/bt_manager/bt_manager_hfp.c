...
static uint32_t _bt_manager_call_to_hfp_volume(uint32_t call_vol)
{
	...
	if (call_vol == 0) {
		hfp_vol = 0;
	} else if (call_vol >= audio_policy_get_max_volume(AUDIO_STREAM_VOICE)) {
		hfp_vol = 0x0F;
	} else {
		hfp_vol = (call_vol * 0x10 / audio_policy_get_max_volume(AUDIO_STREAM_VOICE));
		if (hfp_vol == 0) {
			hfp_vol = 1;
		}
	}
...
static uint32_t _bt_manager_hfp_to_call_volume(uint32_t hfp_vol)
{
	...
	if (hfp_vol == 0) {
		...
		call_vol = audio_policy_get_max_volume(AUDIO_STREAM_VOICE);
	} else {
		call_vol = (hfp_vol + 1) * audio_policy_get_max_volume(AUDIO_STREAM_VOICE) / 0x10;
...
{
    uint16_t  last_status = hfp_manager.hfp_status;
	...
	if (hfp_dev)
	{
		if (last_status >= BT_STATUS_INCOMING &&
			state <= BT_STATUS_HFP_NONE)
		{
			/* 通话结束后如果 SCO 一段时间还未断开时, 小机再主动断开?
			 */
			bt_manager_hfp_status_changed(hfp_dev);
		}
	}
...
{
	if(btif_hfp_get_dev_num() == 1)
	{
		if(hfp_manager.hfp_status == BT_STATUS_MULTIPARTY || hfp_manager.hfp_status == BT_STATUS_3WAYIN ||
			hfp_manager.hfp_status == BT_STATUS_3WAYOUT)
		{
			int state = bt_manager_hfp_get_call_state(1);
			int new_status = hfp_manager.hfp_status;			
			if (state != BTSRV_HFP_STATE_CALL_3WAYIN && state != BTSRV_HFP_STATE_CALL_MULTIPARTY)
			{
				switch(state)
				{
					case BTSRV_HFP_STATE_LINKED: new_status = BT_STATUS_HFP_NONE;break;
					case BTSRV_HFP_STATE_CALL_INCOMING: new_status = BT_STATUS_INCOMING;break;
					case BTSRV_HFP_STATE_CALL_OUTCOMING: new_status = BT_STATUS_OUTGOING;break;
					case BTSRV_HFP_STATE_CALL_ALERTED: new_status = BT_STATUS_OUTGOING;break;
					case BTSRV_HFP_STATE_CALL_ONGOING: new_status = BT_STATUS_ONGOING;break;
					default:break;
				}
				SYS_LOG_INF("hfp %d --> %d",  hfp_manager.hfp_status,new_status);
				hfp_manager.hfp_status = new_status;			
			}
		}
	}
...
static int btmgr_hfp_sync_cb_get_state(void)
{
	...
	info.btcall_app = strcmp(APP_ID_BTCALL, app_manager_get_current_app()) ? 0 : 1;
...
static void btmgr_hfp_sync_cb_set_state(uint16_t hdl, void *param)
{
	...
	SYS_LOG_INF("Hfp sync cb %d %d %d 0x%x", info.btcall_app, info.siri_mode, info.only_sco, info.hfp_status);

#if 0	/* Check switch app in btmgr_swapp_check_switch */
	if (info.btcall_app) {
		if (hfp_manager.allow_sco && strcmp("btcall", app_manager_get_current_app())) {
...
static int _bt_manager_hfp_callback(uint16_t hdl, btsrv_hfp_event_e event, uint8_t *param, int param_size)
{
	...
	{
		dev_info->hfp_remote_vol = BT_MANAGER_INVALID_VOLUME;
...
#ifdef CONFIG_BT_TRANSCEIVER
        if (is_ongoing == 1 && bt_manager_config_pts_test()) {
	...
	case BTSRV_HFP_CODEC_INFO:
	{
		...
		SYS_LOG_INF("codec_id: %d sample_rate: %d", hfp_codec_id, hfp_sample_rate);
	...
	case BTSRV_HFP_VOLUME_CHANGE:
	{
		...
		if ((hfp_manager.sco_connect_time && (pass_time < 500))
			|| strcmp(APP_ID_BTCALL, app_manager_get_current_app())) {
	...
	case BTSRV_HFP_SIRI_STATE_CHANGE:
	{
		...
		rep.handle = hdl;
		rep.index = 1;
		SYS_LOG_INF("siri state: %d, hfp state:%d", state,bt_manager_hfp_get_status());
	...
	case BTSRV_SCO_DISCONNECTED:
	{
		...
		if (hfp_manager.only_sco && !strcmp(APP_ID_BTCALL, app_manager_get_current_app())) {
			hfp_manager.only_sco = 0;
...
int bt_manager_hfp_volume_control(uint8_t type, uint8_t volume)
{
	...
	if(!cfg_feature->sp_hfp_vol_syn){
		SYS_LOG_INF("unsupport hfp vol sync");		
		return 0;
	}
...
{
	if (bt_manager_config_support_battery_report() == false){
		return 0;
	}

	if (mode == BT_BATTERY_REPORT_VAL) {
        /* Divide 10 levels, 0 for 10%, 9 for 100%. */
        bat_val = bat_val / 10;
		if (bat_val > 9) {
			bat_val = 9;
		}
...
{
	btmgr_feature_cfg_t *cfg_feature = bt_manager_get_feature_config();
	if(!cfg_feature->sp_hfp_3way_call){
		SYS_LOG_WRN("unsupport 3way call");
		return 0;
	}
	if (!bt_manager_hfp_check_atcmd_time()) {
		return -1;
...
int bt_manager_hfp_start_siri(void)
{
	int ret = 0;
	btmgr_feature_cfg_t *cfg_feature = bt_manager_get_feature_config();
	if(!cfg_feature->sp_hfp_siri){
		SYS_LOG_WRN("unsupport siri");		
		return 0;
	}
	if (!bt_manager_hfp_check_atcmd_time()) {
		return -1;
...
int bt_manager_hfp_sync_vol_to_remote_by_addr(uint16_t hdl, uint32_t call_vol)
{
    ...
    if (cfg->hfp_origin_volume_sync_to_remote &&
		k_work_delayable_is_pending(&dev_info->hfp_vol_to_remote_timer.work))
    {
        SYS_LOG_INF("hfp vol to remote pending\n");
        return 0;
    }

    if (call_vol <= audio_policy_get_max_volume(AUDIO_STREAM_VOICE))
    {
...
void bt_manager_hfp_sync_origin_vol_proc(os_work *work)
{
	...
	SYS_LOG_INF("dev_info->hdl:0x%x",dev_info->hdl);
