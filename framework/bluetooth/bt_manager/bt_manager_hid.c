...
static void bt_manager_hid_callback(uint16_t hdl, btsrv_hid_event_e event, void *packet, int size)
{
	...
	case BTSRV_HID_GET_REPORT:
		...
		bt_manager_hid_send_ctrl_data(hdl, info->type, info->last_report, info->data_len);
		break;
...
{
	bt_mgr_dev_info_t *info = CONTAINER_OF(work, bt_mgr_dev_info_t, hid_delay_work);
...
{
	bt_mgr_dev_info_t *info = bt_mgr_find_dev_info_by_hdl(hdl);
