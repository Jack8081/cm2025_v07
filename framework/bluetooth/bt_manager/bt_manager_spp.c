...
struct btmgr_uuid_cb spp_uuid_cb[CFG_BT_MAX_SPP_CHANNEL];
...
static struct btmgr_uuid_cb *bt_manager_spp_find(uint8_t app_id, uint8_t *uuid)
{
	...
	for (i = 0; i < CFG_BT_MAX_SPP_CHANNEL; i++) {
		if (app_id) {
			...
			if (spp_uuid_cb[i].uuid && !memcmp(spp_uuid_cb[i].uuid, uuid, SPP_UUID_LEN)) {
				spp_info = &spp_uuid_cb[i];
				break;
			}
		} else if(uuid == NULL){
...
{
	struct btmgr_uuid_cb *spp_info = bt_manager_spp_find(0, uuid);
...
static struct btmgr_uuid_cb *bt_manager_snoop_spp_connected_find(uint8_t app_id)
{
	uint8_t i, index;

	for (i = 0; i < CFG_BT_MAX_SPP_CHANNEL; i++) {
		if (spp_uuid_cb[i].app_id == app_id) {
			return &spp_uuid_cb[i];
		}
	}

	index = MGRSPP_ID_TO_INDEX(app_id);
	if (!spp_uuid_cb[index].app_id) {
		spp_uuid_cb[index].app_id = MGRSPP_INDEX_TO_ID(i);
		return &spp_uuid_cb[index];
	} else {
		return NULL;
	}
}
...
static void bt_manager_spp_callback(btsrv_spp_event_e event, uint8_t app_id, void *packet, int size)
{
	...
	if (event == BTSRV_SPP_SNOOP_CONNECTED) {
		spp_info = bt_manager_snoop_spp_connected_find(app_id);
	} else {
	...
	switch (event) {
		...
		case BTSRV_SPP_DATA_INDICATED:
			...
			break;
		case BTSRV_SPP_SNOOP_CONNECTED:
			param = packet;
			SYS_LOG_INF("BTSRV_SPP_SNOOP_CONNECTED");
			if (!spp_info->connected) {
				spp_info->connected = 1;
				spp_info->active_connect = (*param) ? 1 : 0;
				/* For active do spp connect, app need reg cb and uuid to the spp_info */
				/* App need process snoop device spp connected if callbak*/
				//if (spp_info->cb && spp_info->cb->connected) {
				//	spp_info->cb->connected(spp_info->app_id, spp_info->uuid);
				//}
			}
			break;
		case BTSRV_SPP_SNOOP_DISCONNECTED:
			SYS_LOG_INF("BTSRV_SPP_SNOOP_DISCONNECTED");
			if (spp_info->connected) {
				spp_info->connected = 0;
				/* App need process snoop device spp disconnected if callbak*/
				//if (spp_info->cb && spp_info->cb->disconnected) {
				//	spp_info->cb->disconnected(spp_info->app_id);
				//}
