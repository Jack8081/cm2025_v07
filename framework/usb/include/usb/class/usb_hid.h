...
int hid_int_ep_write(const uint8_t *data, uint32_t data_len, uint32_t *bytes_ret);

/* read data from hid interrupt endpoint */
int hid_int_ep_read(uint8_t *data, uint32_t data_len, uint32_t *bytes_ret);

/* Initialize USB HID composite devie */
int usb_hid_composite_init(void);

/* register HID device(register report descriptor) */
void usb_hid_register_device(const uint8_t *desc, size_t size,
