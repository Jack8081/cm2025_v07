...
void dsp_session_dump_stat_info(struct dsp_session *session)
{
	...
	dsp_session_dump_decoder_stat_info(session);
}
...
void dsp_session_dump_enc_stat_info(struct dsp_session *session)
{
	...
	dsp_session_dump_encoder_stat_info(session);
}
...
static int dsp_session_message_handler(struct dsp_message *message)
{
	...
	case DSP_EVENT_NEW_DATA:
		type = message->param2 >> 28;
		level = message->param2 & 0x01;
		switch(type) {
		case DSP_DATA_TYPE_ENCODE:
			pin = DEBUG_DSP_GPIO_ENCODE;
			break;
		case DSP_DATA_TYPE_DECODE:
			pin = DEBUG_DSP_GPIO_DECODE;
			break;
		case DSP_DATA_TYPE_PREDEAL:
			pin = DEBUG_DSP_GPIO_PRE_DEAL;
			break;
		case DSP_DATA_TYPE_POSTDEAL:
			pin = DEBUG_DSP_GPIO_POST_DEAL;
			break;
		default:
			pin = 0;
			break;
		...
		if(pin) {
			...
			{
				//debug_gpio_high(pin);
				debug_gpio_display(pin, type, 4, 0);
				//debug_gpio_display(pin, (message->param2 >> 4) & 0xff, 8, 0);		//serial no
				//debug_gpio_display(pin, (message->param2 >> 12) & 0xfff, 12, 1);	//input stream len
			}
			else
			{
				debug_gpio_low(pin);
				//debug_gpio_display(pin, (message->param2 >> 4) & 0xff, 8, 0);		//serial no
				//debug_gpio_display(pin, (message->param2 >> 12) & 0xfff, 12, 0);	//input stream len
			}
		}
		k_sem_give(&global_session->sem);
		break;
...
#define DSP_PROM_ADDR 0x01038000
#define DSP_DROM_ADDR 0x01058000
...
int dsp_session_submit_command(struct dsp_session *session, struct dsp_command *command)
{
	...
	//SYS_LOG_INF("submit cmd %d %d %d %d seq %d totalsize %d \n", command->id, command->data[0], command->data[1],command->size,command->seq,ACTS_RINGBUF_NELEM(size));
	/* insert command */
