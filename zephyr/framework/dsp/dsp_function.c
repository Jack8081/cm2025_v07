...
#if 0 /*FIXME*/
void dsp_session_set_freqadj(struct dsp_session *session, struct freqadj_set_params *freqadj_set_prms)
{
	if (freqadj_set_prms->low_freq_run_change_flag) {
		uint16_t freqadj_prms = *(volatile uint16_t *)(DSP_FREQADJ_ENABLE_REGISTER_BASE);

		freqadj_prms &= ~(0x1<<1);
		freqadj_prms |= (freqadj_set_prms->low_freq_run<<1);
		freqadj_prms &= ~(0x1f<<2);
		freqadj_prms |= (freqadj_set_prms->low_run<<2);
		freqadj_prms &= ~(0x1f<<7);
		freqadj_prms |= (freqadj_set_prms->high_run<<7);

		*(volatile uint16_t *)(DSP_FREQADJ_ENABLE_REGISTER_BASE) = freqadj_prms;
	}
}
#endif

void dsp_session_dump_decoder_stat_info(struct dsp_session *session)
{
	struct decoder_dspfunc_info *decoder_info;
	...
	decoder_info = request.info;

	if (decoder_info->params.format == DSP_CODEC_FORMAT_NAV) {
		struct dec_nav_stat_info *p_nav_info = &(decoder_info->debug.dec_stat_info.nav_info);
		if (p_nav_info->stat_info_flag == 1) {
			int i;

			printk("xfer stat, in %d packets, miss: %d\n", p_nav_info->xfer_pkt_total, p_nav_info->xfer_miss_total);
			for (i = 0; i < 16; i++) {
				if (p_nav_info->xfer_rt_count[i] > 0) {
					printk("  rt[%d]: %d\n", i, p_nav_info->xfer_rt_count[i]);
				}
			}
			if (p_nav_info->xfer_no_pkt_count > 0) {
				printk("  no pkt: %d\n", p_nav_info->xfer_no_pkt_count);
			}
			if (p_nav_info->xfer_crc_err_count > 0) {
				printk("  crcx pkt: %d\n", p_nav_info->xfer_crc_err_count);
			}
			if (p_nav_info->xfer_null > 0) {
				printk("  null pkt: %d\n", p_nav_info->xfer_null);
			}

			p_nav_info->stat_info_flag = 0;
		}
	}
}

void dsp_session_dump_encoder_stat_info(struct dsp_session *session)
{
	struct encoder_dspfunc_info *encoder_info;
	struct dsp_request_function request = { .id = DSP_FUNCTION_ENCODER, };
	dsp_request_userinfo(session->dev, DSP_REQUEST_FUNCTION_INFO, &request);

	if (request.info == NULL)
		return;

	encoder_info = request.info;

	if (encoder_info->params.format == DSP_CODEC_FORMAT_NAV) {
		struct enc_nav_stat_info *p_nav_info = &(encoder_info->debug.enc_stat_info.nav_info);
		if (p_nav_info->stat_info_flag == 1) {
			int i;

			printk("send stat, in %d packets, miss: %d\n", p_nav_info->send_pkt_total, p_nav_info->send_fail_total);
			for (i = 0; i < 16; i++) {
				if (p_nav_info->send_rt_count[i] > 0) {
					printk("  rt[%d]: %d\n", i, p_nav_info->send_rt_count[i]);
				}
			}
			if (p_nav_info->send_prefetch_fail_count > 0) {
				printk("  prefetchx: %d\n", p_nav_info->send_prefetch_fail_count);
			}
...
int dsp_session_get_fade_info(struct dsp_session *session, uint16_t *fade_sta)
{
	struct dsp_request_function request = { .id = DSP_FUNCTION_POSTPROCESS, };
	dsp_request_userinfo(session->dev, DSP_REQUEST_FUNCTION_INFO, &request);
	...
	struct streamout_dspfunc_info *streamout_info = (struct streamout_dspfunc_info *)(request.info);
	*fade_sta = streamout_info->runtime.fade_sta;
	return 0;
}

int dsp_session_get_encoder_latency_info(struct dsp_session *session, struct encoder_dspfunc_runtime_inbuf_latency *inbuf_latency)
{
	struct dsp_request_function request = { .id = DSP_FUNCTION_ENCODER, };
	dsp_request_userinfo(session->dev, DSP_REQUEST_FUNCTION_INFO, &request);
	...
	struct encoder_dspfunc_info *encoder_info = (struct encoder_dspfunc_info *)(request.info);
	struct encoder_dspfunc_runtime_inbuf_latency *p = &(encoder_info->runtime.inbuf_latency);

	if (p->dsp_write_seq != p->cpu_read_seq && p->dsp_write_seq == (uint16_t)(p->cpu_read_seq + 1)) {
		memcpy(inbuf_latency, p, sizeof(struct encoder_dspfunc_runtime_inbuf_latency));
		p->cpu_read_seq = p->dsp_write_seq;
		return 0;
	}
