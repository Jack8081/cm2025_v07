...
#define PROFILE_DEBUG_INFO_INDEX_START (8)
...
/**
 * @brief get session run fade info
 *
 * @param session Address of session
 *
 * @param fade_sta Address of fade info
 *
 * @return 0 if succeed, the others if failed
 */
...
/**
 * @brief get encoder session run inbuf latency info
 *
 * @param session Address of session
 *
 * @param session Address of inbuf latency info
 *
 * @return 0 if succeed, the others if failed
 */
...
/**
 * @brief dump session statistics information
 *
 * @param session Address of session
 *
 * @return N/A
 */
void dsp_session_dump_stat_info(struct dsp_session *session);

/**
 * @brief dump session encoder statistics information
 *
 * @param session Address of session
 *
 * @return N/A
 */
void dsp_session_dump_enc_stat_info(struct dsp_session *session);

struct freqadj_set_params {
	uint32_t global_enable_change_flag : 1;
	uint32_t global_enable : 1;
	uint32_t low_freq_run_change_flag : 1;
	uint32_t low_freq_run : 1;
	uint32_t high_run : 5;
	uint32_t low_run : 5;
	uint32_t high_freq : 8;
};
...
/**
 * @brief Pre Enable a session function with command DSP_CMD_FUNCTION_PRE_ENABLE.
 *
 * This routine pre enable a function using command DSP_CMD_FUNCTION_PRE_ENABLE without params.
 *
 * @param session Address of the session.
 * @param func Session function ID.
 * @param sem Address of semaphore object for synchronization.
 ...
 * @return Command result.
 */
static inline int dsp_session_pre_enable_func_sync(struct dsp_session *session, unsigned int func, struct k_sem *sem)
{
	struct dsp_command *command = dsp_command_alloc(DSP_CMD_FUNCTION_PRE_ENABLE, sem, 4, &func);
	int res = -ENOMEM;
	if (command) {
		res = dsp_session_submit_command(session, command);
		dsp_command_free(command);
 ...
 * @return Command result.
 */
static inline int dsp_session_pre_enable_func(struct dsp_session *session, unsigned int func)
{
	return dsp_session_pre_enable_func_sync(session, func, NULL);
}
