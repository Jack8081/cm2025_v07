...
void* hal_aout_channel_open(audio_out_init_param_t *init_param)
{
	...
	aout_param.audiopll_mode = init_param->audiopll_mode;
	aout_param.automute_force = init_param->automute_force;
...
int hal_aout_channel_set_aps(void *aout_channel_handle, unsigned int aps_level, unsigned int aps_mode)
{
    ...
    {
        return audio_out_control(audio_out->aout_dev, aout_channel_handle, AOUT_CMD_SET_APS_F, &aps_level);
    }
...
int hal_aout_set_fifo_src(void *aout_channel_handle, u8_t channel_id, bool from_dsp, void *dsp_audio_set_param, void *dsp_audio_handle)
{
	hal_audio_out_context_t*  audio_out = _hal_audio_out_get_context();
	dac_fifosrc_setting_t dac_fifosrc = {
		...
		.dsp_audio_handle    = dsp_audio_handle,
	};
...
int hal_aout_channel_set_dac_enable(void *aout_channel_handle, dac_ext_trigger_ctl_t *trigger_ctl)
{
	hal_audio_out_context_t*  audio_out = _hal_audio_out_get_context();

	assert(audio_out->aout_dev);

    return audio_out_control(audio_out->aout_dev, aout_channel_handle, AOUT_CMD_DAC_FORCE_START, trigger_ctl);
}
