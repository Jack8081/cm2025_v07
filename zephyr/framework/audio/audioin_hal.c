...
void* hal_ain_channel_open(audio_in_init_param_t *init_param)
{
	...
	ain_param.channel_width = (init_param->data_width == 24)? (CHANNEL_WIDTH_24BITS): (CHANNEL_WIDTH_16BITS);
	...
	ain_param.disable_discard = init_param->disable_discard;
	ain_param.audiopll_mode  = init_param->audiopll_mode;
	...
	switch (init_param->channel_type) {
		case AUDIO_CHANNEL_ADC:
		{
            ...
            {
                adc_setting.gain.ch_again[i] = init_param->input_gain;
                adc_setting.gain.ch_dgain[i] = 0;
            }
#else
...
int hal_ain_channel_set_drq_level(void* ain_channel_handle, uint8_t *drq_level)
{
	int result;
	...
	result = audio_in_control(audio_in->ain_dev, ain_channel_handle, AIN_CMD_SET_ADC_FIFO_DRQ_LEVEL, drq_level);
...
int hal_ain_channel_set_aps(void *ain_channel_handle, unsigned int aps_level, unsigned int aps_mode)
{
    ...
    if (aps_mode == APS_LEVEL_AUDIOPLL_F) {
        return audio_in_control(audio_in->ain_dev, ain_channel_handle, AIN_CMD_SET_APS_F, &aps_level);
    } else {
	    return audio_in_control(audio_in->ain_dev, ain_channel_handle, AIN_CMD_SET_APS, &aps_level);
    }
}
