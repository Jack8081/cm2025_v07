...
static void * lvgl_fs_open(struct _lv_fs_drv_t *drv,
		...
		uint32_t pos, lv_fs_whence_t whence)
{
	...
	err = fs_seek((struct fs_file_t *)file, pos, (int)whence);
	return errno_to_lv_fs_res(err);
}
...
static void * lvgl_fs_dir_open(struct _lv_fs_drv_t *drv,
		const char *path)
{
	struct fs_dir_t *dir;
	int err;
