...
static uint32_t read_efuse_freq_value(uint32_t *read_cap_value, int *index)
{
    ...
    for (i = 0; i < MAX_EFUSE_CAP_RECORD; i++) {
        ...
        ret_val = soc_atp_get_hosc_calib(i, (unsigned int *)(&cap_value.value[i]));
#else
        ret_val = soc_get_hosc_calib_sim(i, (unsigned int *)(&cap_value.value[i]));
#endif
        if(ret_val){
            *read_cap_value = 0xff;
            *index = 0xff;		
...
static int32_t spi_nor_freq_compensation_param_read(uint32_t *trim_cap)
{
    ...
    int ret = nvram_config_get("BT_CFO_VAL", temp, sizeof(temp));
...
static int32_t spi_nor_freq_compensation_param_write(uint32_t *trim_cap)
{
    ...
    snprintf(temp, sizeof(temp), "%d", *trim_cap);

#ifdef CONFIG_PROPERTY
    return nvram_config_set_factory("BT_CFO_VAL", temp, strlen(temp) + 1);
#else
    return 0;
#endif
}
