				   ...
				   int key_code, int value)
{
	...
	if (adckey->notify) {
		...
		adckey->notify((struct device *)adckey->this_dev, &val);
	}
}
...
#endif
{
...
#ifdef CONFIG_CFG_DRV
    if (adckey->print_adcval)
        printk("adcval 0x%x, keycode %d\n", adcval, keycode);
#else
	...
	if (keycode == adckey->prev_keycode) {
		adckey->scan_count++;
    #ifdef CONFIG_CFG_DRV
        if (adckey->scan_count >= (adckey->debounce_ms / cfg->poll_interval_ms))
    #else
		...
		{	/* previous key is released? */
				...
				&& keycode != adckey->prev_stable_keycode)
		    {
		        ...
		        /* support lradc combo key for earphone: A -> AB -> B */
		        if (keycode == KEY_RESERVED)
		    #endif
		        {
...
#ifdef CONFIG_CFG_DRV
    if (adckey->adc_chan != PMUADC_ID_LRADC1)
#else
...
#else
	/* 
	 * if irq come while the work being canceled, 
	 * submit work with K_NO_WAIT would fail, so add 1 cycle delay
	 */
	k_delayed_work_submit(&adckey->timer, K_CYC(1));
#endif
}
...
static int adckey_acts_config_init(const struct device *dev, uint8_t *channel_id)
{
	...
	cfg_get_by_key(ITEM_LRADC_PULL_UP, &LRADC_Pull_Up, sizeof(uint8_t));
	cfg_get_by_key(ITEM_LRADC_VALUE_TEST, &adckey->print_adcval, sizeof(uint8_t));
	cfg_get_by_key(ITEM_LRADC_DEBOUNCE_TIME_MS, &adckey->debounce_ms, sizeof(uint16_t));

	if (ret > 0) {
		uint8_t channel_no = LRADC_Ctrl & 0xff;
		uint8_t gpio_no = (LRADC_Ctrl & 0xff00) >> 8;
		uint8_t mfp_sel = (LRADC_Ctrl & 0xff0000) >> 16;
		uint32_t pin_mode = 0;
		...
		adckey->adc_chan = PMUADC_ID_LRADC1 + channel_no - 1;
		*channel_id = adckey->adc_chan;

		acts_pinmux_get(gpio_no, &pin_mode);
		
		pin_mode = (pin_mode & ~GPIO_CTL_MFP_MASK) | (mfp_sel << GPIO_CTL_MFP_SHIFT);
		pin_mode = (pin_mode & ~GPIO_CTL_PULL_MASK) | (LRADC_Pull_Up << GPIO_CTL_PULL_SHIFT);
...
int adckey_acts_init(const struct device *dev)
{
	...
	adckey->sequence.channels = BIT(channel_cfg.channel_id);
	adckey->sequence.buffer = &adckey->adc_buf[0];
	adckey->sequence.buffer_size = sizeof(adckey->adc_buf);
	adckey->this_dev = dev;

    if(channel_cfg.channel_id == 5){  //onlay LRADC1 can trigger pmu interrupt
	    if (soc_pmu_register_notify(&detect_param)) {
		    LOG_ERR("failed to register pmu notify");
		    return -ENXIO;
	    }
