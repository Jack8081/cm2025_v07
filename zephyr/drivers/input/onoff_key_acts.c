...
//#define SUPPORT_INNER_TOUCH_KEY 1
...
#if SUPPORT_INNER_TOUCH_KEY
/*! 
 * \brief Disable inner Touch Key 
 */ 
static void sys_ctrl_touch_key_disable(void) 
{ 
    int  i; 
    ...
    sys_write32(0, CTK_CTL);
    sys_write32(0, CTK_VSET);
} 

/*! 
 * \brief Enable inner Touch Key 
 */ 
static void sys_ctrl_touch_key_enable(void) 
{ 
    int  i; 

    for (i = 0; i < 20; i++) 
    { 
        sys_write32(0xc0000009, CTK_CTL);
        sys_write32(0xc000000d, CTK_CTL);
    } 
    
    
    sys_write32(0xc0000001, CTK_CTL);
    sys_write32(0, CTK_CTL);
} 
#endif
...
static int onoff_key_acts_init(const struct device *dev)
{
    ...
    if (cfg_get_by_key(ITEM_ONOFF_PULLR_SEL,
            &pullr_sel, sizeof(pullr_sel))) {
        soc_pmu_config_onoffkey_pullr_sel(pullr_sel);
    }
