...
#if (BOARD_TYPE == BOARD_LARK)
#define UART_TX_PIN  (GPIO_28)
#define UART_RX_PIN  (GPIO_29)
#else
#define UART_TX_PIN  (GPIO_5)
#define UART_RX_PIN  (GPIO_6)
#endif
...
static void acts_pinmux_setup_pins_dynamic_uart(void)
{
	...
	pinconf[0].pin_num = UART_TX_PIN;
	pinconf[0].mode = UART0_MFP_CFG;
	pinconf[1].pin_num = UART_RX_PIN;
	pinconf[1].mode = UART0_MFP_CFG;
...
#ifdef CONFIG_CFG_DRV
		if(strcmp(CONFIG_UART_CONSOLE_ON_DEV_NAME, dev->name) == 0) {
			...
			if(strcmp(CONFIG_UART_DC5V_ON_DEV_NAME, dev->name) != 0) {
				printk("uart: %s\n\n\n", dev->name);
