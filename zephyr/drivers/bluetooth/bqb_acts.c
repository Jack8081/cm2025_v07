...
static void bt_dut_mode_set_name(void)
{
	...
	ret = property_get(cfg_name, &bqb_info->tx_data[3], (BT_NAME_LEN-1));
	if (ret < 0) {
		return;
	}

	bqb_info->tx_data[0] = 0x13;
	bqb_info->tx_data[1] = 0x0c;
	bqb_info->tx_data[2] = (uint8_t)(ret + 2);

	bqb_info->tx_data[3 + ret] = 0x00;
	bqb_info->tx_data[3 + ret + 1] = 0x00;
	btdrv_send(HCI_CMD, bqb_info->tx_data, (uint16_t)(3 + ret + 2));/* HCI set name */
	k_sleep(K_MSEC(20));
...
static int bqb_uart_init(void)
{
	...
	extern void __printk_set_null(int print_null);
...
{
#ifdef CONFIG_SOC_SERIES_CUCKOO
	#define BT_UART_MFP_SEL 3
	struct acts_pin_config bqb_pin_cfg[] = {
		{7, BT_UART_MFP_SEL | GPIO_CTL_PADDRV_LEVEL(1) | GPIO_CTL_PULLUP},
...
#endif		
	struct btc_debug_config* pbtc_usr_config = ipmsg_get_btc_debug_config();
		
	bqb_pin_cfg[0].pin_num = pbtc_usr_config->BTC_BQB_TX_Pin;
	bqb_pin_cfg[1].pin_num = pbtc_usr_config->BTC_BQB_RX_Pin;
...
int bqb_init(int bqb_mode /*0:BR BQB Test, 1:BLE BQB Test,2:BR/BLE dual BQB Test*/)
{
    int err;
    ...
    bqb_info->ble_bqb_mode = bqb_mode ? 1 : 0;
    ...
    if (bqb_info->ble_bqb_mode) {
		...
		if (bqb_mode == 1)
			return 0;
    	/* Support br/ble BQB test in one config */
    }
