...
void bat_charge_ctrl_enable(uint32_t current_sel, uint32_t voltage_sel)
{
    ...
    if (t->is_valid) {
		...
		if(bat_charge->charge_cv_offset < 0x80) {
			...
			//maybe need use +200mv
			if(adjust_volt > 0x1f) {
				cv_need_add_200mv = 1;
				adjust_volt = adjust_volt - 16;    //200mv/12.5mv=16
			}
		} else {
    ...
    LOG_INF("cur:0x%x vol:0x%x adjust_vol:0x%x add_200:%d", current_sel, voltage_sel, adjust_volt, cv_need_add_200mv);
...
uint16_t bat_charge_get_charge_stop_current(void)
{
	bat_charge_context_t *bat_charge = bat_charge_get_context();
	uint8_t index = bat_charge->configs.cfg_charge.Charge_Stop_Current;
	uint16_t stop_current_ma;

	switch(index) {
		case CHARGE_STOP_CURRENT_20_PERCENT:
			stop_current_ma = bat_charge_get_current_ma(bat_charge->configs.cfg_charge.Charge_Current) * 20 / 100;
	        break;

		case 	CHARGE_STOP_CURRENT_5_PERCENT:
			stop_current_ma = bat_charge_get_current_ma(bat_charge->configs.cfg_charge.Charge_Current) * 5 / 100;
			break;
		...
		default:
			stop_current_ma = bat_charge_get_current_ma(bat_charge->configs.cfg_charge.Charge_Current) * 10 / 100;
			break;
	}

	LOG_INF("charge stop current: %dma", stop_current_ma);

    return stop_current_ma;
}
...
#else
    adjust_current->stop_current_ma = bat_charge_get_charge_stop_current();
#endif
