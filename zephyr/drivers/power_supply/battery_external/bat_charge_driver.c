...
uint32_t bat_check_voltage_change(uint32_t bat_volt)
{
    ...
    if((bat_charge->dc5v_current_state == DC5V_STATE_IN) && (bat_charge->bat_charge_current_state < BAT_CHG_STATE_FULL) && \
...
void bat_charge_timer_handler(struct k_work *work)
{
    ...
    if (bat_charge->bat_adc_sample_timer_count >= (10000 / BAT_CHARGE_DRIVER_TIMER_MS)) {
            bat_charge->bat_adc_sample_timer_count = 0;
...
#else
            bat_v_mv = get_battery_voltage_by_coulometer();
#endif

            LOG_INF("state:%d <%s>, dc5v:%d <%s>, bat:%dmv",
            ...
            //LOG_INF("ctl: 0x%x dc5v_d: 0x%x bat_d: 0x%x", sys_read32(0x40004200), sys_read32(0x40004218), sys_read32(0x40004214));
            LOG_INF("info: 0x%x, 0x%x, 0x%x", ext_charger_dump_info1(), ext_charger_dump_info2(), ext_charger_check_enable());
    }
	...
	k_delayed_work_submit_to_queue(work_queue, &bat_charge->timer, K_MSEC(BAT_CHARGE_DRIVER_TIMER_MS));
}
