...
bool spi_data_status;
void spi_acts_set_recv_mode(bool pram)
{
    spi_data_status = pram;
}

bool spi_acts_get_recv_mode(void)
{
    return spi_data_status;
}
...
int spi_acts_write_read_data(const struct acts_spi_config *cfg, struct acts_spi_data *data,
    const uint8_t *tx_buf, uint8_t *rx_buf, int len)
{
    int ret;
...
int spi_acts_transfer_data(const struct acts_spi_config *cfg, struct acts_spi_data *data)
{
    ...
    if (ctx->tx_len && ctx->rx_len) {
        ...
        ret = spi_acts_write_read_data(cfg, data, ctx->tx_buf, ctx->rx_buf, chunk_size);
                ...
                const struct spi_config *config)
{
    ...
    if(spi_acts_get_recv_mode())
    {
        ctrl = SPI_CTL_DELAYCHAIN(3);
    ...
    spi_acts_set_clk(cfg, config->frequency);
