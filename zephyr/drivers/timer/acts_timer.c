...
{
	if(cycle < CYC_PER_TICK/2)
		cycle = CYC_PER_TICK/2;
	if(cycle > MAX_CYCLES)
		cycle = MAX_CYCLES;
...
void sys_clock_compensate(uint32_t ticks);
...
int sys_clock_device_ctrl(const struct device *device, enum pm_device_action action)
{
	if(action == PM_DEVICE_ACTION_RESUME){
		...
		if(sleep_st_cycle){
			...
			uint32_t ticks = (uint64_t)cycle * RC32K_MUTIPLE / CYC_PER_TICK;
            sys_clock_compensate(ticks);
			printk("--t0-wakeup=%d ms\n", ticks * 1000 / Z_HZ_ticks);
			
            last_cycle_count = sys_read32(T0_CNT);
			acts_timer_set_next_irq(CYC_PER_TICK);
		}		
