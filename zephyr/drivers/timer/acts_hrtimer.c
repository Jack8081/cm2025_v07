...
static uint64_t _get_current_jiffies(void *p_ctx)
{
#ifdef CONFIG_SOC_USE_SYS_ROM_CODE
	return p_rom_timeout_api->p__get_current_jiffies(p_ctx);
#else
    uint32_t val;
    uint32_t flags;
...
void hrtimer_init_s(struct hrtimer *ttimer, hrtimer_expiry_t expiry_fn,
		       void *expiry_fn_arg)
{
    hrtimer_init(ttimer, expiry_fn, expiry_fn_arg);
    ttimer->sleep_wake = 1;
}
...
void hrtimer_start(struct hrtimer *ttimer, s32_t duration, s32_t period)
{
	...
	current_jiffies = _get_current_jiffies(_get_hrtimer_ctx());
...
static enum WK_RUN_TYPE wk_run_tpye;

static enum WK_CB_RC sleep_wk_callback(enum S_WK_SRC_TYPE wk_src)
{    
    if (wk_run_tpye == WK_RUN_IN_NOR)
    {
        sys_write32(0x0, CMU_TIMER1CLK); //select hosc
        hrtimer_handle_expired();
...
__sleepfunc static enum WK_RUN_TYPE sleep_wk_prepare(enum S_WK_SRC_TYPE wk_src)
{
    ...
    wk_run_tpye = WK_RUN_IN_NOR;
    current_jiffies = ctx->current_jiffies + ctx->timer_load_val;
    SYS_DLIST_FOR_EACH_CONTAINER_SAFE(&ctx->hrtimer_q, cur, tmpNode, node){
        if (cur->expiry_time > current_jiffies) {
            break;
        }

        if (!cur->sleep_wake) {
            wk_run_tpye = WK_RUN_IN_SYTEM;
            break;
        }
    }
...
static struct sleep_wk_fun_data sleep_fn = 
{
...
static int hrtimer_runtime_resume(void)
{
    ...
    // t1 wakeup
    sys_s3_wksrc_clr(SLEEP_WK_SRC_T1);
...
static void hrtimer_runtime_suspend(void)
{
    ...
    ctx->current_jiffies = _get_current_jiffies(ctx);
    ...
    sleep_register_wk_callback(SLEEP_WK_SRC_T1, &sleep_fn);
