...
#define PWM_PIN_CYCLES_PER_SEC		(16000)
...
#ifdef CONFIG_SOC_LARK
#define PWM_MFP						(0x12 | (0x1 << 5) | ((3) << GPIO_CTL_PADDRV_SHIFT))
#else
#define PWM_MFP						(8 | (0x1 << 5) | ((3) << GPIO_CTL_PADDRV_SHIFT))
#endif
...
#else
    struct acts_pin_config pins_pwm_mfp[4];
#endif
};
			     ...
			     u32_t period_cycles, u32_t pulse_cycles, pwm_flags_t flags)
{
	...
	LOG_DBG("PWM@%d set period cycles %d, pulse cycles %d",
...
static int pwm_acts_pin_set_duty(const struct device *dev, u32_t chan,
			     u32_t pulse_cycles)
{
	const struct pwm_acts_config *cfg = dev->config;
	//struct pwm_acts_data *data = dev->data;
	...
	LOG_DBG("PWM@%d set pulse cycles %d", chan, pulse_cycles);
...
const struct pwm_driver_api pwm_acts_driver_api = {
	...
	//	.set_program = pwm_acts_set_program_mode,
		...
		.pin_set_duty = pwm_acts_pin_set_duty,
};
