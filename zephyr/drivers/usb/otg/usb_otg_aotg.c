...
static inline void aotg_dc_isr_dispatch(uint8_t vector)
{
	...
	case UIV_EP2OUT:
	case UIV_EP3OUT:
#if (CONFIG_USB_AOTG_OTG_VERSION == USB_AOTG_VERSION_LARK)
	case UIV_EP4OUT:
#endif
...
{
    printk("clear disconnect\n");
	usb_clear_bit8(USBCS, USBCS_DISCONN);
    printk("clear disconnect end\n");
}
...
{
	return ((usb_read8(DPDMCTRL) & BIT(LSDETEN)) &&
		!(usb_read8(LINESTATUS) & LINESTATE_DM));
}
