...
typedef struct {
	...
	uint8_t disable_discard : 1; /* disable discard several samples at the beginning of adc */
} ain_session_t;
					   ...
					   uint32_t channel, int status)
{
	...
	uint32_t ret_reson;
	uint32_t key, delta;
    //uint32_t len = session->reload_len / 2;
	...
	if ((!already_discard) && (session->channel_type == AUDIO_CHANNEL_ADC) && time) {
	...
	if (session && AIN_SESSION_CHECK_MAGIC(session->magic)) {
		if (session->callback) {
			...
			if (status == DMA_IRQ_HF) {
				...
				if (!session->disable_discard && !already_discard && time){
...
static void *acts_audio_in_open(struct device *dev, ain_param_t *param)
{
	...
	session->disable_discard = param->disable_discard;
...
static int acts_audio_in_control(struct device *dev, void *handle, int cmd, void *param)
{
    ...
    if (cmd == AIN_CMD_ANC_AAL_CONTROL) {
        audio_in_lock(dev);
        ret = phy_audio_control(data->adc0_dev, AIN_CMD_ANC_AAL_CONTROL, param);
        audio_in_unlock(dev);
        return ret;
    }
...
static int acts_audio_in_start(struct device *dev, void *handle)
{
	...
	if (AUDIO_CHANNEL_ADC == session->channel_type) {
		...
		if (data->adc_sess_stack_cur > 0) {
            ...
            ret = phy_audio_control(data->adc0_dev, PHY_CMD_ADC_FIFO_ENABLE, NULL);
			if (ret) {
				LOG_ERR("Failed to enable ADC fifo err=%d", ret);
				return ret;
			} 
