...
#ifdef CONFIG_SOC_SERIES_CUCKOO_FPGA
	sr_khz *= 4;
#endif
...
int audio_pll_get_aps_f(a_pll_type_e index)
{
	...
	if (AUDIOPLL_TYPE_0 == index) {
		...
		reg = sys_read32(AUDIO_PLL0_CTL) & CMU_AUDIOPLL0_CTL_APS0_F_MASK;
		if (reg >= (AUDIOPLL_APS_FRACTIONAL_MAX + 1)) {
			/* 48KHz sample rate seires */
			reg -= (AUDIOPLL_APS_FRACTIONAL_MAX + 1);
		}
	} else {
		LOG_ERR("Invalid AUDIO_PLL type %d", index);
		return -EINVAL;
...
int audio_pll_set_aps(a_pll_type_e index, audio_aps_level_e level)
{
	...
	if (AUDIOPLL_TYPE_0 == index) {
		...
		if (APS_LEVEL_0PPM == level) {
			if ((reg  & CMU_AUDIOPLL0_CTL_APS0_MASK) >= 8) {
				level = 0x18;
			} else {
				level = 0x10;
			}
...
int audio_pll_set_aps_f(a_pll_type_e index, unsigned int level)
{
	...
	if (level > AUDIOPLL_APS_FRACTIONAL_MAX) {
		LOG_ERR("Invalid APS_F level setting %d", level);
		return -EINVAL;
	}
	...
	if (AUDIOPLL_TYPE_0 == index) {
        ...
        reg = sys_read32(AUDIO_PLL0_CTL);
        if ((reg  & CMU_AUDIOPLL0_CTL_APS0_F_MASK) >= 32){ /* 48KHz sample rate seires */
            level += 32;
        }
        reg &= ~CMU_AUDIOPLL0_CTL_APS0_F_MASK;
        reg |= CMU_AUDIOPLL0_CTL_MODE_F;
        sys_write32(reg | CMU_AUDIOPLL0_CTL_APS0_F(level), AUDIO_PLL0_CTL);
	} else {
		LOG_ERR("Invalid AUDIO_PLL type %d", index);
		return -EINVAL;
	}
...
/* @brief audio pll set */
static void audio_pll_set(a_pll_type_e index, a_pll_series_e series, a_pll_mode_e pll_mode)
{
	uint32_t reg;
	...
	if (AUDIOPLL_TYPE_0 == index) {
		...
		LOG_DBG("AUDIOPLL_DEBUG - 0x%x", sys_read32(AUDIOPLL_DEBUG));
	} else{
		LOG_ERR("set pll error!");
	}
}
...
int audio_pll_check_config(a_pll_series_e series, uint8_t *index, a_pll_mode_e pll_mode)
{
	...
	*index = pll_index;
	...
	audio_pll_set(pll_index, series, pll_mode);
