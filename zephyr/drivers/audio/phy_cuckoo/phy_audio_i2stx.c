                ...
                a_i2stx_mclk_clksrc_e mclk_src, bool direct_set)
{
    ...
    int ret;
    uint8_t clk_div, pll_index, fs_sel=0;
    uint32_t reg = 0, reg1;
    uint32_t lrclk_div;
    a_mclk_type_e mclk;
    ...
    if (CLK_SRCTX_I2STX_EXT != mclk_src) {
        ...
        if (!PHY_DEV_FEATURE(mode)) {
            ...
            reg |= CMU_I2STXCLK_I2STXBLRCLKOEN;
        } else {
    ...
    if ((CLK_SRCTX_DAC_256FS == mclk_src)
        ...
        uint8_t clk_div=0, firclkdiv=0, fir2xclkdiv=0, cicclkdiv=0, pll_index=0;
        ...
        switch(sr_khz){
            ...
            case(SAMPLE_RATE_12KHZ):{clk_div=0; firclkdiv=0; fir2xclkdiv=1; cicclkdiv=1; fs_sel=AUDIOPLL_48KSR; break;}  //48K
            ...
            case(SAMPLE_RATE_48KHZ):{clk_div=1; firclkdiv=1; fir2xclkdiv=1; cicclkdiv=0; fs_sel=AUDIOPLL_48KSR; break;}  //12K
        ...
        reg1 = sys_read32(CMU_DACCLK) & (~0x1FFFFFF);
        
        /* Select audio_pll0 or audio_pll1 */
        reg1 |= (pll_index & 0x1) << CMU_DACCLK_DACCLKSRC_SHIFT;
        reg1 |= CMU_DACCLK_DACSDMLCLKEN;
        reg1 |= CMU_DACCLK_DACCICFIRCLKEN;
        reg1 |= CMU_DACCLK_DACFIFO0CLKEN;
        reg1 |=  (clk_div << CMU_DACCLK_DACCLKDIV_SHIFT);
        reg1 |= (firclkdiv << CMU_DACCLK_DACFIRCLKDIV_SHIFT);
        reg1 |= CMU_DACCLK_DACFIFCLK2XDIV(fir2xclkdiv);
        reg1 |= (cicclkdiv << CMU_DACCLK_DACCICCLKDIV_SHIFT);
        
        sys_write32(reg1, CMU_DACCLK);

    } else if (CLK_SRCTX_I2STX == mclk_src) {
        /* Get audio PLL setting  */
        ret = audio_get_pll_setting_i2s(sr_khz, mclk, &clk_div, &fs_sel);
        if (ret) {
...
static int i2stx_sample_rate_get(struct device *dev)
{
    ...
    if ((CLK_SRCTX_DAC_256FS == mclk_src)
        ...
        ret = audio_get_pll_sample_rate(clk_div, pll_index);
    } else if (CLK_SRCTX_I2STX == mclk_src) {
        reg = sys_read32(CMU_I2STXCLK);
        clk_div = reg & 0xF;
        pll_index = 0;
...
/* @brief Get the AUDIO_PLL APS used by I2STX */
static int i2stx_get_pll_aps_f(struct device *dev)
{
    uint32_t reg;
    uint8_t pll_index, mclk_src;
    int ret = -1;
...
/* @brief Set the AUDIO_PLL APS_F used by I2STX */
static int i2stx_set_pll_aps_f(struct device *dev, unsigned int level)
{
    uint32_t reg;
    uint8_t pll_index, mclk_src;
    int ret = -1;
...
/* @brief Update the fifo source */
static int i2stx_fifo_update_src(struct device *dev, dac_fifosrc_setting_t *fifosrc)
{
    struct phy_i2stx_drv_data *data = dev->data;
    struct acts_audio_i2stx *i2stx_reg = get_i2stx_reg_base(dev);
...
{
    struct acts_audio_i2stx *i2stx_reg = get_i2stx_reg_base(dev);
    i2stx_reg->fifoctl &= ~I2STX0_FIFOCTL_FIFO_RST;
    i2stx_reg->fifoctl |= I2STX0_FIFOCTL_FIFO_RST;
}
...
static void i2stx_fifo_enable(struct device *dev, audio_fifouse_sel_e sel, audio_dma_width_e width, bool this_use)
{
    ...
    /* If the use dac link mode, the FIFO_SEL shall set to DAC FIFO source, otherwise will get no dma IRQ */
    if (!this_use)
        reg = I2STX0_FIFOCTL_FIFO_SEL; /* 0: I2STX FIFO; 1: DAC FIFO */

    if (DMA_WIDTH_16BITS == width)
        reg |= I2STX0_FIFOCTL_TXFIFO_DMAWIDTH;
...
static void i2stx_digital_enable(struct device *dev, audio_ch_width_e width, bool en)
{
    ...
    if (PHY_DEV_FEATURE(channel_num) != 2) {
        fmt = 3; /* TDM mode */
        reg &= ~ (0xF << I2STX0_CTL_TDMTX_SYNC_SHIFT);

        if (PHY_DEV_FEATURE(channel_num) == 8)
            reg |= I2STX0_CTL_TDMTX_CHAN; /* 0: 4 channels; 1: 8 channels */

        if (PHY_DEV_FEATURE(tdm_format))
            reg |= I2STX0_CTL_TDMTX_MODE; /* 0: I2S format; 1: Left-justified format */

        reg |= I2STX0_CTL_TDMTX_SYNC(PHY_DEV_FEATURE(tdm_frame));
    }

    reg |= (I2STX0_CTL_TXMODELSEL(fmt) | I2STX0_CTL_TXWIDTH(tx_width));

    if (PHY_DEV_FEATURE(bclk_width))
        reg |= I2STX0_CTL_TXBCLKSET;

    i2stx_reg->tx_ctl = reg;

    /* I2S TX enable */
    if (en)
        i2stx_reg->tx_ctl |= I2STX0_CTL_TXEN;
}
...
static int i2stx_srd_cfg(struct device *dev, uint8_t srd_th, audio_i2s_srd_period_e period, bool irq_en, bool mute)
{
    ...
    reg = (I2STX0_SRDCTL_SRD_TH(srd_th & 0x7) | I2STX0_SRDCTL_CNT_TIM(period));

    if (irq_en)
        reg |= I2STX0_SRDCTL_SRD_IE;

    if (mute)
        reg |= I2STX0_SRDCTL_MUTE_EN;

    i2stx_reg->srdctl = reg;

    /* enable slave mode sample rate detect */
    i2stx_reg->srdctl |= I2STX0_SRDCTL_SRD_EN;
    ...
    _wl = i2stx_reg->srdstat & I2STX0_SRDSTA_WL_MASK;
    while (_wl != wl) {
...
static uint32_t read_i2stx_srd_count(struct device *dev)
{
    ...
    return ((i2stx_reg->srdstat & I2STX0_SRDSTA_CNT_MASK) >> I2STX0_SRDSTA_CNT_SHIFT);
}
...
void i2stx_srd_wl_change(struct device *dev)
{
    ...
    width = i2stx_reg->srdstat & I2STX0_SRDSTA_WL_MASK;
...
static int phy_i2stx_enable(struct device *dev, void *param)
{
    ...
    if (out_param->channel_type & AUDIO_CHANNEL_DAC) {
        ...
        if (AOUT_FIFO_DAC0 == out_param->outfifo_type) {
            ...
            i2stx_reg->fifoctl |= I2STX0_FIFOCTL_FIFO_SEL;
        } else {
...
static int phy_i2stx_ioctl(struct device *dev, uint32_t cmd, void *param)
{
    ...
    case AOUT_CMD_SET_APS_F:
    {
        unsigned int level = *(unsigned int *)param;
        ret = i2stx_set_pll_aps_f(dev, level);
        if (ret) {
            LOG_ERR("Failed to set audio pll APS err=%d", ret);
            return ret;
        }
    ...
    case AOUT_CMD_SET_FIFO_SRC:
    {
        ...
        ret = i2stx_fifo_update_src(dev, fifosrc);
        break;
    }
...
static void phy_i2stx_isr(const void *arg)
{
    ...
    if (i2stx_reg->srdstat & I2STX0_SRDSTA_TO_PD) {
        i2stx_reg->srdstat |= I2STX0_SRDSTA_TO_PD;
        if (data->srd_callback)
            data->srd_callback(data->cb_data, I2STX_SRD_TIMEOUT, NULL);
    }

    /* Sample rate changed detection irq pending */
    if (i2stx_reg->srdstat & I2STX0_SRDSTA_SRC_PD) {
        i2stx_reg->srdstat |= I2STX0_SRDSTA_SRC_PD;
        i2stx_srd_fs_change(dev);
    }

    /* Channel width change irq pending */
    if (i2stx_reg->srdstat & I2STX0_SRDSTA_CHW_PD) {
        i2stx_reg->srdstat |= I2STX0_SRDSTA_CHW_PD;
