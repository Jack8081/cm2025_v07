...
static void i2srx_dump_register(struct device *dev)
{
    ...
    LOG_INF("  GPIO_17: %08x", sys_read32(0x40068048));
    LOG_INF("  GPIO_18: %08x", sys_read32(0x4006804c));
    LOG_INF("  GPIO_19: %08x", sys_read32(0x40068050));
    LOG_INF("  GPIO_20: %08x", sys_read32(0x40068054));
}
                ...
                a_i2srx_mclk_clksrc_e mclk_src)
{
    ...
    printk("++++%s,%d, mclk_src:%d\n", __func__, __LINE__, mclk_src);
    if (CLK_SRCRX_I2STX == mclk_src) {
        ...
        sys_write32(sys_read32(CMU_I2SRXCLK) | CMU_I2SRXCLK_I2SRXCLKSRC, CMU_I2SRXCLK);
        ...
        if (!PHY_DEV_FEATURE(mode)) {
            ...
            reg |= CMU_I2SRXCLK_I2SRXBLRCLKOEN;
        } else {
        ...
        ret = audio_pll_check_config(series, &pll_index, data->audiopll_mode);
        if (ret) {
...
/* @brief Get the AUDIO_PLL APS used by I2SRX */
static int i2srx_get_pll_aps_f(struct device *dev)
{
    uint32_t reg;
    uint8_t pll_index, mclk_src;
    int ret = -1;
    ...
    if (CLK_SRCRX_I2SRX == mclk_src) {
        pll_index = 0;
        ret = audio_pll_get_aps_f((a_pll_type_e)pll_index);
    } else if (CLK_SRCRX_I2STX == mclk_src) {
        pll_index = 0;
        ret = audio_pll_get_aps_f((a_pll_type_e)pll_index);
    }if (CLK_SRCRX_I2SRX_EXT == mclk_src) {
...
/* @brief Set the AUDIO_PLL APS_F used by I2SRX */
static int i2srx_set_pll_aps_f(struct device *dev, unsigned int level)
{
    uint32_t reg;
    uint8_t pll_index, mclk_src;
    int ret = -1;
    ...
    if (CLK_SRCRX_I2SRX == mclk_src) {
        pll_index = 0;
        ret = audio_pll_set_aps_f((a_pll_type_e)pll_index, level);
    } else if (CLK_SRCRX_I2STX == mclk_src) {
        pll_index = 0;
        ret = audio_pll_set_aps_f((a_pll_type_e)pll_index, level);
    } else if (CLK_SRCRX_I2SRX_EXT == mclk_src) {
...
{
    struct acts_audio_i2srx *i2srx_reg = get_i2srx_reg_base(dev);
    i2srx_reg->fifoctl &= ~I2SRX0_CTL_RXEN;
}
...
static void i2srx_fifo_enable(struct device *dev, audio_fifouse_sel_e sel, audio_dma_width_e width)
{
    ...
    if (DMA_WIDTH_16BITS == width)
        reg |= I2SRX0_FIFOCTL_RXFIFO_DMAWIDTH;

    if (FIFO_SEL_CPU == sel) {
        reg |= (I2SRX0_FIFOCTL_RXFFIE | I2SRX0_FIFOCTL_RXFRT);
    } else if (FIFO_SEL_DMA == sel) {
        reg |= (I2SRX0_FIFOCTL_RXFFDE | I2SRX0_FIFOCTL_RXFRT
                | I2SRX0_FIFOCTL_RXFOS(1));
    } else if (FIFO_SEL_DSP == sel) {
        reg |= I2SRX0_FIFOCTL_RXFOS(3) | I2SRX0_FIFOCTL_RXFRT;
    } else {
...
static void i2srx_digital_enable(struct device *dev, audio_ch_width_e width)
{
    ...
    if (PHY_DEV_FEATURE(channel_num) != 2) {
        fmt = 3; /* TDM mode */
        reg &= ~ (0xF << I2SRX0_CTL_TDMRX_SYNC_SHIFT);

        if (PHY_DEV_FEATURE(channel_num) == 8)
            reg |= I2SRX0_CTL_TDMRX_CHAN;

        if (PHY_DEV_FEATURE(tdm_format))
            reg |= I2SRX0_CTL_TDMRX_MODE;

        reg |= I2SRX0_CTL_TDMRX_SYNC(PHY_DEV_FEATURE(tdm_frame));
    }

    reg |= (I2SRX0_CTL_RXMODELSEL(fmt) | I2SRX0_CTL_RXWIDTH(rx_width));

    if (PHY_DEV_FEATURE(bclk_width))
        reg |= I2SRX0_CTL_RXBCLKSET;
...
static uint32_t read_i2srx_srd_count(struct device *dev)
{
    ...
    return ((i2srx_reg->srdstat & I2SRX0_SRDSTA_CNT_MASK) >> I2SRX0_SRDSTA_CNT_SHIFT);
}
...
void i2srx_srd_wl_change(struct device *dev)
{
    ...
    width = i2srx_reg->srdstat & I2SRX0_SRDSTA_WL_MASK;
...
static int i2srx_srd_cfg(struct device *dev, uint8_t srd_th, audio_i2s_srd_period_e period, bool irq_en, bool mute)
{
    ...
    reg = (I2SRX0_SRDCTL_SRD_TH(srd_th & 0x7) | I2SRX0_SRDCTL_SRD_TH(period));
    if (irq_en)
        reg |= I2SRX0_SRDCTL_SRD_IE;
    if (mute)
        reg |= I2SRX0_SRDCTL_MUTE_EN;

    i2srx_reg->srdctl = reg;

    /* enable slave mode sample rate detect */
    i2srx_reg->srdctl |= I2SRX0_SRDCTL_SRD_EN;
    ...
    _wl = i2srx_reg->srdstat & I2SRX0_SRDSTA_WL_MASK;
    while (_wl != wl) {
    ...
    i2srx_reg->srdstat |= I2SRX0_SRDSTA_CHW_PD;
...
static int phy_i2srx_ioctl(struct device *dev, uint32_t cmd, void *param)
{
    ...
    case AIN_CMD_SET_APS_F:
    {
        ...
        ret = i2srx_set_pll_aps_f(dev, level);
        if (ret) {
            LOG_ERR("Failed to set audio pll APS err=%d", ret);
            return ret;
        }
...
void phy_i2srx_isr(const void *arg)
{
    ...
    if (i2srx_reg->srdstat & I2SRX0_SRDSTA_TO_PD) {
        i2srx_reg->srdstat |= I2SRX0_SRDSTA_TO_PD;
        if (data->srd_callback)
            data->srd_callback(data->cb_data, I2SRX_SRD_TIMEOUT, NULL);
    }

    /* Sample rate changed detection irq pending */
    if (i2srx_reg->srdstat & I2SRX0_SRDSTA_SRC_PD) {
        i2srx_reg->srdstat |= I2SRX0_SRDSTA_SRC_PD;
        i2srx_srd_fs_change(dev);
    }

    /* Channel width change irq pending */
    if (i2srx_reg->srdstat & I2SRX0_SRDSTA_CHW_PD) {
        i2srx_reg->srdstat |= I2SRX0_SRDSTA_CHW_PD;
