...
LOG_MODULE_REGISTER(aout, CONFIG_LOG_DEFAULT_LEVEL);
...
struct aout_drv_data {
    ...
    uint8_t antipop_processed:1;   /* indicates the antipop is processed.*/
};
...
/* @brief wait the pa operations finish */
static int acts_audio_wait_antipop_complete(struct device *dev, uint32_t timeout_ms)
{
	struct aout_drv_data *data = dev->data;
	uint32_t start_time, curr_time;
    uint8_t is_anti_complete = 0;

	start_time = k_cycle_get_32();

	while (is_anti_complete == 0) {
        if (phy_audio_control(data->dac0_dev, PHY_CMD_GET_ANTIPOP_STAGE, &is_anti_complete)) {
            LOG_ERR("get antipop stage fail");
            return -ETIMEDOUT;
        }
		...
		if (k_cyc_to_us_floor32(curr_time - start_time)
			>= (timeout_ms * 1000)) {
			LOG_ERR("wait antipop stage timeout");
			return -ETIMEDOUT;
		}
...
static void *acts_audio_out_open(struct device *dev, aout_param_t *param)
{
    ...
    if(!data->antipop_processed){
        acts_audio_open_pa(dev, true);
        data->antipop_processed = true;
    }
    
    acts_audio_wait_antipop_complete(dev, AOUT_SESSION_WAIT_PA_TIMEOUT);
#endif
...
static int acts_audio_dma_wait_timeout(struct aout_drv_data *data, aout_session_t *session, uint32_t timeout)
{
	uint32_t start_time, curr_time;

	LOG_DBG("audio wait write dma_chan:%d", session->dma_chan);

	start_time = k_cycle_get_32();

	while (1) {
		if (!__acts_audio_check_dma_busy(data->dma_dev, session->dma_chan))
			break;

		curr_time = k_cycle_get_32();
		if (k_cyc_to_us_floor32(curr_time - start_time) >= timeout) {
			LOG_ERR("wait dma(%d) finish timeout", session->dma_chan);
			return -ETIMEDOUT;
		}

#if (CONFIG_AUDIO_DAC_WAIT_WRITE_PCMBUF_SLEEP_MS > 0)
		if ((!k_is_in_isr())
            && (phy_audio_control(data->dac0_dev, PHY_CMD_GET_FIFO_WORKING, &session->outfifo_type) == 0)
...
#endif
			{
			ret = acts_audio_dma_wait_timeout(data,
					session, CONFIG_AUDIO_DAC_WAIT_WRITE_PCMBUF_TIMEOUT_US);
			if (ret) {
...
/* @brief check the status of all audio out sessions when enter PM suspend */
static int audio_out_pm_check_all_sessions(const struct device *dev)
{
	int i;
	aout_session_t *sess = aout_sessions;
	...
	for (i = 0; i < AOUT_SESSION_MAX; i++, sess++) {
		if (AOUT_SESSION_CHECK_MAGIC(sess->magic)
			&& (sess->flags & AOUT_SESSION_OPEN)) {
			LOG_INF("!!! pm error => session@%p has opened", sess);
			return -EACCES;
		}
	}
...
static int audio_out_pm_control(const struct device *device, enum pm_device_action action)
{
	int ret = 0;
    
	struct aout_drv_data *data = device->data;

	switch (action) {
	case PM_DEVICE_ACTION_RESUME:
		LOG_INF("audio out power resume");
#if (CONFIG_AUDIO_DAC_POWER_PREFERRED == 0)
		//ret = acts_audio_open_pa((struct device *)device, true);
#endif
		break;
	case PM_DEVICE_ACTION_SUSPEND:
		LOG_INF("audio out power suspend");
		ret = audio_out_pm_check_all_sessions(device);
		if (ret)
			return ret;

#if (CONFIG_AUDIO_DAC_POWER_PREFERRED == 0)
		ret = acts_audio_close_pa((struct device *)device);
        data->antipop_processed= false;
#endif
		break;
...
#else
#define audio_out_pm_control NULL
#endif
...
static int audio_out_init(const struct device *dev)
{
    ...
    /*clear antipop_complete indicator*/
	data->antipop_processed = 1;
