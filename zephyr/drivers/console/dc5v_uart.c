...
#else
	//cuckoo or cuckoo_fpga
	printk("pmu_det: 0x%x\n", sys_read32((0x40004000 + 0x0c)));
	printk("ch_ctl: 0x%x\n", sys_read32((0x40004000 + 0x100)));
	printk("wk_ctl: 0x%x\n", sys_read32((0x40004000 + 0x110)));
	printk("wk_pd: 0x%x\n", sys_read32((0x40004000 + 0x114)));
	printk("cmu_s1: 0x%x\n", sys_read32((0x40001000 + 0x70)));
#endif
...
#else
	    dc5v_uart->bak_GPIO_DC5V_CTL |= DC5V_CTL_MFP_UART0RX;
#endif
	}
...
#else
            dc5v_uart->bak_GPIO_DC5V_CTL |= DC5V_CTL_MFP_UART0TX;			
#endif
		}
		//dc5v_uart->bak_GPIO_DC5V_CTL |= DC5V_CTL_10KPU_EN;
...
void dc5v_uart_ctrl_suspend(void)
{
    ...
    if (dc5v_uart->suspended == 0)
    {
        dc5v_uart->suspended = 1;
...
bool dc5v_uart_check_io_time(u32_t ms)
{
    ...
    if ((dc5v_uart->redirect_console_print != 0) &&
        (dc5v_uart->enabled != 0) &&
        (dc5v_uart->suspended == 0))
    {
        return YES;
...
bool dc5v_uart_ctrl_init(CFG_Type_DC5V_UART_Comm_Settings* cfg)
{
    ...
    */
    ...
    {
        dc5v_uart_set_enable((dc5v_uart->enabled = 1));
        dc5v_uart_clear_rx_buf(10);
    }
    else
    {
        dc5v_uart_set_enable((dc5v_uart->enabled = 0));
    }
...
static void dc5v_uart_stop_rxdeal(void)
{
	...
	while (dc5v_uart->rxdeal_quited == 0)
	{
		os_sleep(1);
	}
