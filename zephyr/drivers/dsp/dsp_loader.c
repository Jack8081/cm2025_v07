...
int dsp_acts_request_image(struct device *dev, const struct dsp_imageinfo *image, int type)
{
	...
	if (load_dsp_image_from_file(&image->filp, &entry_point)) {
...
int dsp_acts_handle_image_pagemiss(struct device *dev, uint32_t epc)
{
	...
	/* soft breakpoint : epc = 0x207c626b, dump all memory to debug */
	if (epc == 0x207c626b) {
		extern int dsp_dump_all_dsp_memory_flag;
		dsp_dump_all_dsp_memory_flag = 1;
		return -EACCES;
	}
#endif
...
#ifdef CONFIG_LOAD_IMAGE_FROM_FS
	if (load_dsp_image_bank_from_file(&image->filp, bank_no)) {
