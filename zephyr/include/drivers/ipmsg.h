...
struct btc_debug_config
{
    uint8_t  BTC_Log;         // 蓝牙控制器LOG输出方式,  值为 0 表示关闭, 1 表示通过BTC UART 输出,2表示通过系统打印	 
    uint8_t  BTC_Log_TX_Pin;  // BTC UART 输出管脚
    uint8_t  BTC_Log_RX_Pin;  // BTC UART 输入管脚
    uint8_t  BTC_BQB_TX_Pin;  // BQB UART1 输出管脚
    uint8_t  BTC_BQB_RX_Pin;  // BQB UART1 输入管脚
    uint8_t  BTC_Fix_TX_Power;  //BTC 固定TX POWER, 值为0表示关闭，其它值表示固定TX power的 level值    
    uint8_t  BTC_Debug_Signal;  //BTC 调试信号, 值为0表示禁止，1表示使能    
};
