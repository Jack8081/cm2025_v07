...
struct anc_bootargs {
	uint32_t cmd2anc; /* address of command buffer */
	uint32_t cmd2cpu;
	uint32_t debug_buf;	/*use for debug*/
	uint32_t boot_on;	/*anc dsp boot on flag*/
};
 ...
 */
typedef int (*anc_api_poweron)(struct device *dev, char is_384k);
 ...
 */
static inline int anc_poweron(struct device *dev, char is_384k)
{
	const struct anc_driver_api *api = dev->api;

	return api->poweron(dev, is_384k);
}
