...
#define AIN_CMD_ANC_AAL_CONTROL                                   (22)
/*!< Control to enable or disable AAL function.
 * int audio_in_control(dev, NULL, #AIN_CMD_ANC_AAL_CONTROL, uint8_t *enable)
 * Returns 0 if successful and negative errno code if error.
 */

#define AIN_CMD_AEC_CONTROL                                   (23)
...
#define AIN_CMD_GET_APS_F                                       (24)
/*!< Get the AUDIO_PLL APS_F
 * int audio_in_control(dev, handle, #AIN_CMD_GET_APS_F, unsigned int *aps)
 * Returns 0 if successful and negative errno code if error.
 */

#define AIN_CMD_SET_APS_F                                       (25)
/*!< Set the AUDIO_PLL APS_F for the sample rate tuning
 * int audio_in_control(dev, handle, #AIN_CMD_SET_APS_F, unsigned int *aps)
 * Returns 0 if successful and negative errno code if error.
 */
 ...
 */
typedef struct {
#define ADC_GAIN_INVALID ((int16_t)0xFFFF)
    #if CONFIG_AUDIO_IN_ADGAIN_SUPPORT
	int16_t ch_again[ADC_CH_NUM_MAX];
    int16_t ch_dgain[ADC_CH_NUM_MAX];
    #else
...
#define AIN_DMA_IRQ_TC (1 << 1)
	...
	uint8_t disable_discard;
		/*!< Disable discard several samples at the beginning of adc */
    ...
    a_pll_mode_e audiopll_mode;
       /*The audiopll mode: 0:integer-N; 1:fractional-N*/
} ain_param_t;
