 ...
 * int audio_out_control(dev, handle, #AOUT_CMD_SET_APS, audio_aps_level_e *aps)
 * Returns 0 if successful and negative errno code if error.
 */

#define AOUT_CMD_GET_APS_F                                   (AOUT_FIFO_CMD_FLAG | 18)
/*!< Get the AUDIO_PLL APS
 * int audio_out_control(dev, handle, #AOUT_CMD_GET_APS_F, audio_aps_level_e *aps)
 * Returns 0 if successful and negative errno code if error.
 */

#define AOUT_CMD_SET_APS_F                                     (AOUT_FIFO_CMD_FLAG | 19)
/*!< Set the AUDIO_PLL APS for the sample rate tuning
 * int audio_out_control(dev, handle, #AOUT_CMD_SET_APS_F, audio_aps_level_e *aps)
 * Returns 0 if successful and negative errno code if error.
 */
...
typedef struct {
	...
	void * dsp_audio_handle;
		/*!< The handle of dsp_audio_set_param */
...
#define AOUT_DMA_IRQ_TC         (1 << 1)     /*!< DMA irq transfer completly flag */
    ...
    uint8_t automute_force;
        /*!< The automute force: 0:see CONFIG_AUDIO_DAC_0_AUTOMUTE; 1:automute force enable */
} aout_param_t;
