...
typedef int (*pwm_pin_set_d)(const struct device *dev, uint32_t pwm,
...
__syscall int pwm_pin_set_duty(const struct device *dev, uint32_t pwm,
				 uint32_t pulse);

static inline int z_impl_pwm_pin_set_duty(const struct device *dev,
					    uint32_t pwm, uint32_t pulse)
{
	struct pwm_driver_api *api;

	api = (struct pwm_driver_api *)dev->api;
	return api->pin_set_duty(dev, pwm, pulse);
}
