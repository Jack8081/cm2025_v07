...
enum display_pixel_format {
	...
	PIXEL_FORMAT_ABGR_6666		= BIT(8),

	PIXEL_FORMAT_A8		= BIT(9),
	PIXEL_FORMAT_A4		= BIT(10),
	PIXEL_FORMAT_A1		= BIT(11),
	PIXEL_FORMAT_A4_LE		= BIT(12), /* little endian */
	PIXEL_FORMAT_A1_LE		= BIT(13), /* little endian */

	PIXEL_FORMAT_RGB_565_LE		= BIT(14), /* little endian */
};
 ...
 * @brief Callback API for flushing data to display now
 * See display_flush() for argument description
 */
typedef int (*display_flush_api)(const struct device *dev, int timeout);
...
/**
 * @brief Flush pending data to display.
 *
 * It's mainly used in AOD mode without vsync/TE enabled.
 *
 * @param dev Pointer to device structure
 ...
 * @retval 0 on success else negative errno code.
 */
static inline int display_flush(const struct device *dev, int timeout)
{
	struct display_driver_api *api =
		(struct display_driver_api *)dev->api;

	if (api->flush) {
		return api->flush(dev, timeout);
	}
