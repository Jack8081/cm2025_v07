...
extern const char* const __log_level_str[];
...
#define Z_LOG_TO_PRINTK(_level, fmt, ...) do {  \
		z_log_with_call_addr(_level, fmt "", ##__VA_ARGS__);  \
	} while (false)

#define Z_LOG_WITH_FUNC_NAME(_level, fmt, ...) do {  \
        z_log_with_func_name(_level, __func__, fmt "", ##__VA_ARGS__);  \
    } while (false)

#define Z_LOG_ONLY_FUNC_NAME(_level, fmt, ...) do {  \
        printk("%s%s\n", __log_level_str[_level], __func__);  \
    } while (false)
