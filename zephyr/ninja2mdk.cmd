...
if exist %adir%\%cfgdir%\prj.conf (
	set cfgfile=-DCONF_FILE=%cfgdir:\=/%/prj.conf
) else (
	set cfgfile=
)
cmake -GNinja -DBOARD=%board% -H%adir% -B%bdir% %cfgfile%
...
%GNUARMEMB_TOOLCHAIN_PATH%\bin\arm-none-eabi-gcc.exe ^
	...
	-I%zdir%/kernel/include -I%zdir%/arch/arm/include -I%zdir%/include -I%bdir%/zephyr/include/generated -I%zdir%/drivers -I%zdir%/subsys/tracing/include -I%zdir%/subsys/tracing/sysview -I%sdir%/soc/arm/actions/lark -I%sdir%/../thirdparty/hal/cmsis/CMSIS/Core/Include -I%zdir%/modules/segger -I%zdir%/../thirdparty/debug/segger/SEGGER -I%zdir%/../thirdparty/debug/segger/Config -I%sdir%/../thirdparty/debug/segger/systemview -isystem %zdir%/lib/libc/minimal/include -isystem %GNUARMEMB_TOOLCHAIN_PATH%/lib/gcc/arm-none-eabi/9.3.1/include ^
	-Os -imacros%bdir%/zephyr/include/generated/autoconf.h -ffreestanding -fno-common -g -mcpu=cortex-m4 -mthumb -mabi=aapcs -imacros%zdir%/include/toolchain/zephyr_stdint.h -Wall -Wformat -Wformat-security -Wno-format-zero-length -Wno-main -Wno-address-of-packed-member -Wno-pointer-sign -Wpointer-arith -Wno-unused-but-set-variable -Werror=implicit-int -fno-asynchronous-unwind-tables -fno-pie -fno-pic -fno-strict-overflow -fno-reorder-functions -fno-defer-pop  -ffunction-sections -fdata-sections -std=c99 -nostdinc ^
...
:GEN_PACK_BAT
	...
	if "%topdir%" == "application" (
		set zbase=%zbase%..\..\..\..\..\zephyr
	) else (
		set zbase=%zbase%..\..\..\..\..
	)
	if exist .build_config (
		copy /Y /B .build_config %3\
	)
	echo @echo off > %3\PackCommand.bat
	echo mkdir ..\zephyr >> %3\PackCommand.bat
	echo copy /Y /B zephyr\.config ..\zephyr\ >> %3\PackCommand.bat
	echo copy /Y /B zephyr.bin ..\zephyr\ >> %3\PackCommand.bat
	echo %zbase%\scripts\kallsyms_win.exe --fbin=ksym.bin ^< ksym.map ^> ksym.S >> %3\PackCommand.bat
	echo copy /Y /B ksym.bin ..\zephyr\ >> %3\PackCommand.bat
	echo if exist .build_config ( copy /Y /B .build_config %zbase%\ ) >> %3\PackCommand.bat
	echo cd %zbase% >> %3\PackCommand.bat
	if "%topdir%" == "application" (
		echo python build.py -p >> %3\PackCommand.bat
	) else (
		echo python build.py -t -p >> %3\PackCommand.bat
	)
goto:eof