...
static int fatfs_unlink_cluster(struct fs_mount_t *mountp, const char *path, unsigned int cluster, unsigned int blk_ofs)
{
	int res = -ENOTSUP;

#if !defined(CONFIG_FS_FATFS_READ_ONLY)
	res = f_unlink_cluster(&path[1], cluster, blk_ofs);
...
static int fatfs_readdir(struct fs_dir_t *zdp, struct fs_dirent *entry)
{
	...
	if (res == FR_OK) {
		...
		if (entry->name[0] != 0) {
			...
			entry->start_cluster = fno.start_cluster;
...
static int fatfs_open_cluster(struct fs_file_t *zfp, char *dir, unsigned int cluster, unsigned int blk_ofs)
{
	FRESULT res;
	uint8_t fs_mode;
	void *ptr;
...
static const struct fs_file_system_t fatfs_fs = {
	...
	.unlink_cluster = fatfs_unlink_cluster,
};
