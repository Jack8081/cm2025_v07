...
extern void lvgl_heap_dump(void);

static int shell_cmd_lvglheap_dump(const struct shell *shell, size_t argc, char **argv)
{
	lvgl_heap_dump();
	return 0;
}
...
extern int shell_cmd_dsp_console(const struct shell *shell, size_t argc, char **argv);
extern int tr_bt_vs_set_debug_params(u32_t dbg_enable, u32_t dbg_cfg);

/*
Cuckoo: （内部：22，21，20，19，8，7）（外部：4，10，17，18）
内部信号：22：modem_tx，21：modem_rx，20：sync_win，19：sync_det，8：irq，7：uart_32M
dbg con_debug 1 0x3f 0x0000f003 0x11110a04 0x00001212
将信号DEBUG_CPU_GPIO_PCM_IN					1<<0输出到gpio4
将信号DEBUG_CPU_GPIO_PCM_OUT					1<<1输出到gpio10
将信号DEBUG_DSP_GPIO_DECODE					1<<12输出到gpio17
将信号DEBUG_DSP_GPIO_ENCODE					1<<13输出到gpio17
将信号DEBUG_DSP_GPIO_PRE_DEAL					1<<14输出到gpio18
将信号DEBUG_DSP_GPIO_POST_DEAL					1<<15输出到gpio18
*/
static int shell_cmd_con_debug(const struct shell *shell, int argc, char *argv[])
{
	int enable, i;
	unsigned int bt_mask, valid_debug_signal;
	unsigned int debug_pin_u32_0, debug_pin_u32_1, debug_pin_u32_2, debug_pin_u32_3;
	unsigned char debug_pin[16];
	...
	bt_mask = strtol(argv[2], NULL, 16);
	valid_debug_signal = strtoul(argv[3], NULL, 16);
	
    debug_pin_u32_0 = 0x00000000;
    debug_pin_u32_1 = 0x00000000;
    debug_pin_u32_2 = 0x00000000;
    debug_pin_u32_3 = 0x00000000;

    if(argc >= 5) {
        debug_pin_u32_0 = strtoul(argv[4], NULL, 16);
    }
    if(argc >= 6) {
        debug_pin_u32_1 = strtoul(argv[5], NULL, 16);
    }
    if(argc >= 7) {
        debug_pin_u32_2 = strtoul(argv[6], NULL, 16);
    }
    if(argc >= 8) {
        debug_pin_u32_3 = strtoul(argv[7], NULL, 16);
    }
	...
	for(i = 8; i < 12; i++)
	{
		debug_pin[i] = debug_pin_u32_2 & 0xff;
		debug_pin_u32_2 >>= 8;
	}
	for(i = 12; i < 16; i++)
	{
		debug_pin[i] = debug_pin_u32_3 & 0xff;
		debug_pin_u32_3 >>= 8;
	}

	tr_bt_vs_set_debug_params(enable, 0x83300555);
	debug_gpio_enable(enable, bt_mask, valid_debug_signal, debug_pin);

	return 0;
}

static int shell_cmd_le_adv(const struct shell *shell, int argc, char *argv[])
{
	typedef struct {
		uint8_t  val[6];
	} bt_addr_t;
	...
	extern int bt_manager_audio_save_peer_addr(bt_addr_le_t* peer);
	extern int property_cache_flush(const char *name);
	int mode;
	bt_addr_le_t peer = {0};
	
	mode = strtol(argv[1], NULL, 16);

	if(mode == 1) {
		bt_manager_audio_save_peer_addr(&peer);
		property_cache_flush(NULL);
		sys_pm_reboot(REBOOT_TYPE_NORMAL);
	}
...
static int shell_dvfs_info(const struct shell *shell, int argc, char *argv[])
{
    dvfs_dump_tbl();
...
static int shell_dvfs_set(const struct shell *shell, int argc, char *argv[])
{
    int level_id;
    ...
    level_id = strtol(argv[1], NULL, 10);

    dvfs_set_level(level_id, "dvfsdebug");

    return 0;
}

static int shell_dvfs_unset(const struct shell *shell, int argc, char *argv[])
{
    int level_id;
    ...
    dvfs_unset_level(level_id, "dvfsdebug");
...
#ifdef CONFIG_DVFS_DYNAMIC_LEVEL
    SHELL_CMD(dvfs_info, NULL, "dump DVFS info", shell_dvfs_info),
    SHELL_CMD(dvfs_set, NULL, "DVFS set", shell_dvfs_set),
    SHELL_CMD(dvfs_unset, NULL, "DVFS unset", shell_dvfs_unset),
#endif
...
#if defined(CONFIG_LVGL)
	SHELL_CMD(lvglheap, NULL, "lvglheap", shell_cmd_lvglheap_dump),
#endif

	SHELL_CMD(dsp, NULL, "dsp help for details", shell_cmd_dsp_console),

	SHELL_CMD(con_debug, NULL, "con_debug", shell_cmd_con_debug),

	SHELL_CMD(le_adv, NULL, "le_adv", shell_cmd_le_adv),
