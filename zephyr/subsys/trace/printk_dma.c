...
static unsigned char __act_s2_notsave dma_printk_buffer[CONFIG_DMA_PRINTK_BUF_SIZE];
...
static int buf_char_out(int c, void *ctx_p)
{
	...
	if(ctx->count >= (CTX_TMP_BUF_LEN-2)){
		#ifdef CONFIG_PRINTK_TIME_FREFIX
		if (g_pr_ctx.isprint_time)
		{		
            g_pr_ctx.last_char = ctx->buf[ctx->count - 1];
		}
        #endif
...
int uart_dma_send_buf_ex(const uint8_t *buf, int len)
{
	printk_ctx_t *pctx = &g_pr_ctx;
	...
	if(!pctx->init)
		...
		if ((pctx->isprint_time) ) {
			ctx_buf.count = get_time_prefix(ctx_buf.buf);
		} else {
			ctx_buf.count = 0;
		}
		if (ctx_buf.count > 0){
			cbuf_output(ctx_buf.buf, ctx_buf.count, pctx);
		}
#endif
...
{
	extern int __printk_get_null_flag(void);

	if (__printk_get_null_flag())
		return;
...
#ifdef CONFIG_CFG_DRV	
	if(!cfg_get_by_key(ITEM_UART_PRINT_TIME_STAMP,	&pctx->isprint_time, 1)){
		printk("default\n");
		pctx->isprint_time = 1;
	}
	printk("time stamp=%d\n", pctx->isprint_time);
