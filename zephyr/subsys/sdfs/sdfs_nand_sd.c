...
//	printk("[sdfs] 0x%x %d\n", adfs_addr, size);
...
#ifdef CONFIG_SDFS_READ_BY_CACHE
			// read flash to cache
			ret = flash_read(dev, (item->cache_off >> 9), sdfs_tmp_buf, (item->cache_len >> 9));
			if(ret < 0)
				return -1;
			// copy to cache
			memcpy(item->cache_buf, sdfs_tmp_buf, item->cache_len);
#else
			ret = flash_read(dev, (item->cache_off >> 9) , item->cache_buf, (item->cache_len >> 9));
			if(ret < 0)
				return -1;
#endif
		}
	...
	while (sec_off < sec_end) {
		...
		ret = flash_read(dev, sec_off, sdfs_tmp_buf, len);
		if(ret < 0)
			return -2;
		sec_off += len;
	...
	if (byte_off > 0) {
		// read flash
		ret = flash_read(dev, sec_off, sdfs_tmp_buf, 1);
		if(ret < 0)
			return -1;
		sec_off += 1;
	...
	if (size >= 512) {
		// read flash
		len = (size >> 9);
		ret = flash_read(dev, sec_off, buf, len);
		if(ret < 0)
			return -1;
		sec_off += len;
		len = (len << 9);
