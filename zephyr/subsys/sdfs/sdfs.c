...
void sdfs_available(unsigned char enable)
{
	b_k_sdfs = enable;
}
...
static int sd_fs_init(const struct device *dev)
{
	...
	int err, crc_is_enabled, rand_is_enabled;
	parti = partition_get_stf_part(STORAGE_ID_NOR, PARTITION_FILE_ID_SDFS_MAP_RES);
	if(parti){
		crc_is_enabled = parti->flag & PARTITION_FLAG_ENABLE_CRC ? 1 : 0;
		rand_is_enabled = parti->flag & PARTITION_FLAG_ENABLE_ENCRYPTION ? 1 : 0;
		err = soc_memctrl_mapping(CONFIG_SD_FS_VADDR_START, parti->file_offset, crc_is_enabled, rand_is_enabled);
		if (err) {
			printk("cannot mapping file_id %d\n", PARTITION_FILE_ID_SDFS_MAP_RES);
			return err;
		}
	}else{
...
/* CFG_DRV：depend on ksdfs, so define sdfs init proi:80, cfg_drv prio: 81, uart drv prio 82  */
