...
static K_MUTEX_DEFINE(trng_mutex);
 ...
 * \return              0
 */
static uint32_t se_trng_process(uint32_t *trng_low, uint32_t *trng_high)
{
...
*/
void se_trng_rand(uint32_t *trng_low, uint32_t *trng_high)
{
	k_mutex_lock(&trng_mutex, K_FOREVER);

	se_init();
	se_trng_process(trng_low, trng_high);
	se_deinit();

	k_mutex_unlock(&trng_mutex);
}
