				...
				struct sleep_wk_cb *cb, int *cb_num)
{
	...
	*cb_num = 0;
			...
			do{
				if(p->wk_prep){
					...
					if(runt == WK_RUN_IN_SRAM){
						if(WK_CB_RUN_SYSTEM == p->wk_cb(i)) //Ҫ���ѵ�ϵͳ��������
							rc = WK_CB_RUN_SYSTEM;
					}else if (runt == WK_RUN_IN_NOR) {// ����nor ��
						if(!b_nor_wk){//nor �˳�idle
							b_nor_wk = true;
	...
	if(rc == WK_CB_SLEEP_AGAIN){
		sys_write32(0xffffffff, NVIC_ICPR0);
		sys_write32(0xffffffff, NVIC_ICPR1);
		if(b_nor_wk){ // ����������ߣ�����nor�Ѿ��˳�idle�����½�idle
			sys_norflash_power_ctrl(1);
		}
	}else{
		if(!b_nor_wk){// ���ϵͳ�ܣ���û���˳�idle���˳�idle
			sys_norflash_power_ctrl(0);
		}
	}
	return rc;
...
void soc_enter_deep_sleep(void)
{
	...
	z_clock_set_timeout(CONFIG_SYS_CLOCK_TICKS_PER_SEC, false);
