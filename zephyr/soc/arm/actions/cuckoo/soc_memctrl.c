...
int soc_memctrl_mapping(u32_t map_addr, u32_t nor_phy_addr, int enable_crc, int enable_rand)
{
	u32_t nor_bus_addr;
	int i;
    ...
    if (enable_rand) {
        nor_bus_addr = (1 << 30) | nor_bus_addr;
    }		
...
void *soc_memctrl_create_temp_mapping(u32_t nor_phy_addr, int enable_crc, int enable_rand)
{
	u32_t tmp_cpu_addr = SOC_MEMCTRL_TEMP_MAP_CPU_ADDR;
	int err;

	printk("%s: create temp mapping 0x%x, nor_phy_addr 0x%x, enable_crc %d, enable_rand %d\n", __func__,
		tmp_cpu_addr, nor_phy_addr, enable_crc, enable_rand);

	err = soc_memctrl_mapping(tmp_cpu_addr, nor_phy_addr, enable_crc, enable_rand);
	if (err < 0)
		return NULL;
