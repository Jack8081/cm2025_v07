...
#define DTCM_SIZE      0xFFFF /*dsp word*/
#define CPU_DTCM_BASE  0x1100000
#define DSP_DTCM_BASE  0

#define PTCM_SIZE	   0x1FFF /*dsp word*/
#define CPU_PTCM_BASE  0x1140000
#define DSP_PTCM_BASE  0

#define DSP_SHARE_MEM_SIZE  0x7FFF /*cpu byte*/
#define CPU_SHARE_BASE  0x1080000
#define DSP_SHARE_BASE  0x40060000

//#define EXT_MEM_SIZE  0x37FFF /*cpu byte*/
/*temporary switch DSP_RAM7 16KBytes to cpu*/
#define EXT_MEM_SIZE  0x33FFF /*cpu byte*/
#define CPU_EXT_BASE  0x1040000
#define DSP_EXT_BASE  0x40000000
#define IMG_BANK_INNER_ADDR(addr) (addr & 0x4003ffff)

#define DSP_RAM_INNER_FLAG   0x64722300
#define DSP_RAM_INNER_MSAK   0xffffff00
...
static inline unsigned int mcu_to_dsp_address(unsigned int mcu_addr, uint8_t addr_type)
{
	...
	/** any dsp ram inner */
	if ((mcu_addr & DSP_RAM_INNER_MSAK) == DSP_RAM_INNER_FLAG) {
		return mcu_addr;
	}
...
static inline unsigned int dsp_to_mcu_address(unsigned int dsp_addr, uint8_t addr_type)
{
	...
	if (dsp_addr >= DSP_SHARE_BASE && dsp_addr <= DSP_SHARE_BASE + DSP_SHARE_MEM_SIZE / 2) {
		...
		/** bank ram */
		if (dsp_addr >= 0x0 && dsp_addr <= EXT_MEM_SIZE / 2) {
...
{
	sys_write32(sys_read32(CMU_DEVCLKEN0) | (1 << 16), CMU_DEVCLKEN0);
	sys_write32(0x1, CMU_DSPCLK);
	while ((sys_read32(CMU_DSPCLK) & (1<<31)) == 0) {
		sys_write32(0x1 | (1<<31), CMU_DSPCLK);
	}
}

static inline void dsp_deinit_clk(void)
{
	while ((sys_read32(CMU_DSPCLK) & (1<<31)) != 0) {
...
//#define DSPRAM_USED_DEF  ((1 << 20) | (1 << 18) | (1 << 16) | (1 << 14) | (1 << 12) | (1 << 10) | (1 << 8) | (1 << 6))
/*temporary switch DSP_RAM7 16KBytes to cpu*/
#define DSPRAM_USED_DEF  ((1 << 18) | (1 << 16) | (1 << 14) | (1 << 12) | (1 << 10) | (1 << 8) | (1 << 6))

static inline int dsp_soc_request_addr(int cpu_addr)
{
	if (cpu_addr >= CPU_EXT_BASE && cpu_addr < CPU_EXT_BASE + EXT_MEM_SIZE) {
		sys_write32(sys_read32(CMU_MEMCLKSRC1) | DSPRAM_USED_DEF, CMU_MEMCLKSRC1);
	}
	return 0;
}
...
{
	sys_write32(sys_read32(CMU_MEMCLKSRC1) | (DSPRAM_USED_DEF | (1 << 0)), CMU_MEMCLKSRC1);
...
static inline int dsp_dump_info(void)
{
	...
	printk("PWRGATE_DIG 0x%x \n",sys_read32(0x40004000 + 0x18));
	printk("CMU_DSPCLK 0x%x \n",sys_read32(0x40001000 + 0x40));
	printk("COREPLL_CTL 0x%x \n",sys_read32(COREPLL_CTL));
	printk("RC64M_CTL 0x%x \n",sys_read32(RC64M_CTL));
