...
#define CONFIG_THREAD_TIMER_DEBUG
//#define THREAD_TIMER_DEBUG
		       ...
		       void *expiry_fn_arg)
{
	...
	__ASSERT(!k_is_in_isr(), "thread timer required.");
...
void thread_timer_start(struct thread_timer *ttimer, s32_t duration, s32_t period)
{
	...
	if (ttimer->tid != _current){
		printk("%s, thread %p: timer %p, func %p\n", __FUNCTION__, (void *)_current,ttimer,ttimer->expiry_fn);
		__ASSERT((ttimer->tid == _current), "");
	}
...
void thread_timer_handle_expired(void)
{
	...
	SYS_DLIST_FOR_EACH_CONTAINER_SAFE(&_current->thread_timer_q, ttimer, next, node) {
		...
		if(ttimer->node.prev == NULL || ttimer->node.next == NULL){
			printk("%s, thread %p: timer %p, func %p\n", __FUNCTION__, (void *)_current, ttimer,
				ttimer->expiry_fn);
			printk("timer=%p, head=%p, next=%p\n", ttimer, ttimer->node.prev, ttimer->node.next);
			_dump_thread_timer_q();		
		}
