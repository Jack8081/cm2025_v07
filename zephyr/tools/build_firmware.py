...
class firmware(object):
    def __init__(self, cfg_file, checkid):
        ...
        if 1 == int(checkid):
            self.checkchipid_in_product = 1
        else:    
            self.checkchipid_in_product = 0
            
        print("chipid check: %d" %(self.checkchipid_in_product))
        ...
        if os.path.isdir(self.ota_dir):
            shutil.rmtree(self.ota_dir)
        os.mkdir(self.ota_dir)
        if os.path.isdir(self.orig_ota_dir):
            shutil.rmtree(self.orig_ota_dir)
    ...
    def parse_config(self, cfg_file, pre_crc = False, pre_encrypt = False):
        ...
        ver_number = "%04x" % fw_ver_number
        if 'version_name' in self.fw_version.keys():
            cur_time = time.strftime('%y%m%d%H%M',time.localtime(time.time()))
            ver_str = '.'.join( (i for i in ver_number) )
            version_name = self.fw_version['version_name'].replace('$(build_time)', cur_time)
            self.fw_version['version_name'] = version_name.replace('$(ver_num)', ver_str)
        ...
        if 'version_code' in self.fw_version.keys():
            ver_str = '0x' + ''.join( ("%02d"%int(i) for i in ver_number) )
            version_code = self.fw_version['version_code'].replace('$(ver_code)', ver_str)
            self.fw_version['version_code'] = version_code
...
#        print(self.partitions)
    ...
    def generate_earphone_config(self, mcfg_file, upg_ini_file):
        with open(mcfg_file, 'w') as f, open(upg_ini_file, 'w') as f_u:
            ...
            if self.checkchipid_in_product == 1:
                f_u.write("checkid 4\n")
            ...
            #add extern data config
            #if soc_name == 'lark':
            #    f.write('\n#include "fw_product_lark.cfg"\n')
            #else:
            #    f.write('\n#include "fw_product_cuckoo.cfg"\n')
            
            f.write('\n#include "fw_product.cfg"\n')
            
            if os.path.isfile(os.path.join(self.bin_dir, "fwimage.cfg")):
                f.write('#include "fwimage.cfg"\n')
            
            #f.write('\nSETPATH=".\\config";\n')
            #f.write('#include "fw_configuration.cfg"\n')

    def __merge_config_bin(self, fw_file, config_bin_file):
        try:
            new_fw_file = "%s_%s.fw" %(os.path.splitext(fw_file)[0],
                os.path.splitext(os.path.basename(config_bin_file))[0])
            cmd = ['ROMModifyCmd.exe', '-make', fw_file, config_bin_file, new_fw_file]
            outmsg, exit_code = run_cmd(cmd)
        except:
            print("Firmware %s merge config binary file %s fail" %(fw_file, config_bin_file))
        
    def merge_config_bin(self, fw_file, config_bin_dir):
        if os.path.isdir(config_bin_dir):
            for parent, dirnames, filenames in os.walk(config_bin_dir):
                for filename in filenames:
                    config_bin_file = os.path.join(parent, filename)
                    if os.path.splitext(config_bin_file)[1].lower() == '.bin':
                        self.__merge_config_bin(fw_file, config_bin_file)
    ...
    def generate_earphone_firmware(self, fw_name):
        ...
        if is_windows():
            #config_bin_dir = os.path.join(self.bin_dir, 'usr_cfg')
            #self.merge_config_bin(fw_file, config_bin_dir)
            config_bin_file = os.path.join(self.bin_dir, 'usr_config.bin')
            print("cfg bin file: %s" %(config_bin_file))
            if os.path.splitext(config_bin_file)[1].lower() == '.bin':
                print("merge config bin!")
                self.__merge_config_bin(fw_file, config_bin_file)
            
        print_notice('Build successfully!')

def main(argv):
    global soc_name, board_name, encrypt_fw, efuse_bin, fw_ver_number
    ...
    parser.add_argument('-v', dest = 'fw_ver_number')
    parser.add_argument('-d', dest = 'check_chipid')
    ...
    fw_ver_number = int(args.fw_ver_number,16)
    date_stamp = time.strftime('%y%m%d',time.localtime(time.time()))
    fw_prefix = board_name  + '_' + date_stamp
    soc_name = args.soc_name
    need_checkid = args.check_chipid

    print('build firmware start, cfgfile: %s' %(fw_cfgfile))
    ...
    try:
        fw = firmware(fw_cfgfile, need_checkid)
